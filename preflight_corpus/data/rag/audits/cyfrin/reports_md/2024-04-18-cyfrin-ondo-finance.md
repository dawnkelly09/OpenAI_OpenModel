...
function test_setInvestorMintLimit_underflow_DoS() public initDefault(alice) {
    ...
    // admin now reduces the mint limit to be under the curren
    // minted amoun
...
function test_mint_zero_address() public {
    ...
    // same issue affects checkAndUpdateRedemptionLimi
}
```
...
### Protocol may be short-changed by `BuidlRedeemer` during a USDC depeg even
...
```solidity
  function checkAndUpdateMintLimitOptimized(
    ...
    if (investorId == 0) {
      ...
      // otherwise execute next code then update investorIdToMintState[investorId].currentAmoun
      ...
      // @audit now update current minted amoun
      ...
      emit MintStateUpdated(
        ...
        0,
        mintAmount,
        defaultMintLimitCache - mintAmoun
      );
    }
    else {
      ...
      emit MintStateUpdated(
        ...
        mintState.limit - mintState.currentAmoun
      );
    }
  }
```
...
```solidity
    // If the address is not being disassociated from all investors, increment the coun
