...
```typescrip
    it("Multiple L2 BridgeTokens can be associated with a single L1 NativeToken by using setCustomContract", async function () {
      ...
      // @audit the setup of this PoC has been copied from the tes
      // "Should mint and burn correctly from a target contract"
      const amountBridged = 100;

      // Deploy custom contrac
      ...
      // @audit deploy a new second custom contrac
      ...
      //
      // @audit Once the suggested fix is applied, this should rever
      ...
      // @audit user ends up with initial token amoun
...
```solidity
  function deployBridgedToken(
    ...
    // @audit removed explicit return statemen
  }
```
...
```solidity
  function bridgeToken(
    ...
    if (nativeToken != EMPTY) {
      ...
      // Send Metadata only when the token has not been deployed on the other chain ye
...
```solidity
  function _addUsedAmount(uint256 _usedAmount) internal {
    ...
    // @audit use `_usedAmount` in assignmen
...
```solidity
  function resetRateLimitAmount(uint256 _amount) external onlyRole(RATE_LIMIT_SETTER_ROLE) {
    ...
    if (currentPeriodEnd < block.timestamp) {
      currentPeriodEnd = block.timestamp + periodInSeconds;
      // @audit remove assignment to `usedAmountResetToZero` and instead directly se
    ...
    // @audit refactor `else/if` statement to simplify
        ...
        // @audit remove assignment usedLimitAmountToSet = _amoun
