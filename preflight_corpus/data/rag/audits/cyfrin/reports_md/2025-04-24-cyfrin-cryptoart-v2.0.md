...
* Creator/Admin can add a `CollectionsStory` for when a collection is added to a contrac
...
```solidity
/// @notice Check on-chain if a token id has a pinned uri or no
/// @dev This call MUST revert if the token does not exis
...
* there is no check for `MintType.Whitelist` and no corresponding whitelist enforcemen
...
### `burn` should delete `tokenURI` related data and emit `TokenUriUnpinned` even
