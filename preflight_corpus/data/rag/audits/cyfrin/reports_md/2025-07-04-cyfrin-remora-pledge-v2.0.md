...
// consider making a similar fix here to prevent overflow rever
...
- As we can see on chisel, 20e18 is > the max value a uint64 can fi
```
...
//@audit-issue => overflow will blow up the tx
            $._holderStatus[payoutForwardAddr].calculatedPayout += SafeCas
...
8. As a result of step 7, the unclaimed payouts earned by the holder get los
...
```solidity
    function test_holderForcesForwarderToLosePayouts() public {
        ...
        // fund total payout amount to funding walle
        ...
        // forwarder claims all the outstanding payou
...
function hasSignedDocs(address signer) public view returns (bool, bytes32) {
    ...
    // skipping to the `return (true, 0x0);` statemen
    for (uint256 i = 0; i < numDocs; ++i) {
        bytes32 docHash = $._docHashes[i];
        if (
            $._documents[docHash].needSignature &&
...
// 200 bits have been used so 56 bits available in the current storage slo
...
### Payout distributions to Remora token holders are diluted by initial token owner min
...
### Overriding fees can't be switched back once se
...
11. As a result of step 10, the payouts earned by the holder get los
...
```solidity
    function test_forwarderLosesHolderPayouts() public {
        ...
        // forwarder claims all his payou
        vm.startPrank(forwarder);
        remoraTokenProxy.claimPayout();
        assertEq(stableCoin.balanceOf(forwarder), forwarderPayoutBalance);
...
```solidity
    function test_transferBricked_whitelistedHolderIsRemovedFromWhitelist() external {
        ...
        // verify from can only transfer the 10 tokens that he received after he was removed from whitelis
        vm.prank(from);
...
1) check `from` balance; if smaller than `amount` required for transfer rever
...
```solidity
    function payoutBalance(address holder) public returns (uint256) {
        ...
        if (
             // @audit must be a holder to claim payouts, prevents forwarders who aren'
             // also holders from claiming their forwarded payouts
            (!rHolderStatus.isHolder) || //non-holder calling the function
            (rHolderStatus.isFrozen && rHolderStatus.frozenIndex == 0) || //user has been frozen from the start, thus no payou
...
function test_forwarderFrozenBeforeFirstPayout_noPayoutBalanceWhileFrozen() external {
    ...
    // forwarder frozen before first distribution payou
    ...
    // forwarder is frozen yet so receives no forwarded payou
...
function test_forwarderFrozenAfterFirstPayout_validPayoutBalanceWhileFrozen_claimPayoutWhileFrozen() external {
    ...
    // forwarder is frozen yet still receives forwarded payou
    uint256 forwarderPayoutBalance = remoraTokenProxy.payoutBalance(forwarder);
    assertEq(forwarderPayoutBalance, payoutDistributionAmount/2);

    // forwarder claims all their payou
    vm.prank(forwarder);
    remoraTokenProxy.claimPayout();
    assertEq(stableCoin.balanceOf(forwarder), forwarderPayoutBalance);
}
...
```solidity
    function test_freezeHolder_setPayoutForwardAddress_toFrozenForwarder() external {
        ...
        // forwarder is frozen  so receives no forwarded payou
        uint256 forwarderPayoutBalance = remoraTokenProxy.payoutBalance(forwarder);
...
### Remove `decimals` from initial `RemoraToken` min
