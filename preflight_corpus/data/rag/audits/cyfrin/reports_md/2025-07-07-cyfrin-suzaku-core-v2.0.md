...
- All subsequent legitimate rebalancing attempts in the same epoch rever
...
```solidity
    function test_DustLimitStakeCausesFakeRebalancing() public {
       ...
       // The key issue: minimal stake removed, but still excess remains
        ...
        middleware.forceUpdateNodes(alice, 0); // Proper rebalancing with no limi
...
```solidity
    function testBlacklistDoesNotBlockMigration() public {
        ...
        vm.prank(alice);
        // This should revert if blacklist was properly enforced, but it won'
...
contract PoCIncorrectSumOfShares is AvalancheL1MiddlewareTest {
    ...
    function _setupRewardsAndSecondaryAssetClass() internal {
        ...
        // Create a secondary asset-class + vault so split is in effec
...
```solidity
   function test_wasActiveAtBoundaryBug() public {
       ...
       // Distribute rewards using actual Rewards contrac
...
contract RewardsAssetShareTest is Test {
    ...
    function test_AssetShareFormula() public {
        ...
        // Wait 3 epochs as required by contrac
        vm.warp((epoch + 3) * middleware.EPOCH_DURATION());
...
- Triggers division by zero in Math.mulDiv(), causing transaction rever
```
...
```solidity
    ...
    // Update the middleware to return the new asset class lis
...
contract MockWarpMessenger is IWarpMessenger {
    ...
    function getVerifiedWarpMessage(
        ...
        // test_ComputeValidatorUptime & test_ValidatorUptimeEven
        if (messageIndex == 0) {
        ...
        // test_ComputeOperatorUptime - first epoch (0) & test_OperatorUptimeEven
...
- No one has the ability to add addresses to the whitelis
...
```solidity
    ...
    function test_WhitelistInconsistency() public {
        // Create a vault with whitelisting enabled but no way to add addresses to the whitelis
        ...
        address vaultAddress = vaultFactory.create(
            ...
            abi.encode(
                IVaultTokenized.InitParams({
                    ...
                    depositWhitelistSetRoleHolder: alice, // Alice can toggle whitelis
                    depositorWhitelistRoleHolder: address(0), // No one can add to whitelis
                    isDepositLimitSetRoleHolder: alice,
                    depositLimitSetRoleHolder: alice,
                    name: "Test",
                    symbol: "TEST"
        ...
        // she has the role to toggle whitelist but not to add addresses to i
        vm.startPrank(alice);
        ...
        // Step 4: Now bob should be able to deposi
        vm.startPrank(bob);
        vault.deposit(bob, 100 ether);
        vm.stopPrank();
...
- No one has the ability to change the deposit limi
...
```solidity
  // This demonstrates that when the vault is created with isDepositLimitSetRoleHolder
    ...
    function test_DepositLimitInconsistency() public {
        // Create a vault with deposit limit enabled but no way to change the limi
        uint64 lastVersion = vaultFactory.lastVersion();
        ...
        address vaultAddress = vaultFactory.create(
            ...
            abi.encode(
                IVaultTokenized.InitParams({
                    ...
                    depositWhitelist: false,
                    isDepositLimit: true, // Deposit limit ENABLED
                    depositLimit: 0, // Zero limi
                    defaultAdminRoleHolder: address(0), // No default admin
                    depositWhitelistSetRoleHolder: alice,
                    depositorWhitelistRoleHolder: alice,
                    isDepositLimitSetRoleHolder: alice, // Alice can toggle limit feature
                    depositLimitSetRoleHolder: address(0), // No one can set the limi
                    name: "Test",
                    symbol: "TEST"
                })
            ),
        ...
        // she can toggle the feature but not set the limi
        vm.startPrank(alice);
...
- Rounding effects in integer arithmetic become significan
...
**Proof of Concept:** Run the following tes
...
contract CheckpointsBugTest is Test {
    ...
    // This test demonstrates the bug when using a valid hin
...
```solidity
    function test_POC_MisattributedStake_NodeIdReused() public {
        ...
        // Use a specific, predictable nodeId for the tes
        ...
        // Ensure operators have some stake in the vaul
...
Output:
```bash
Ran 1 test for test/middleware/AvalancheL1MiddlewareTest.t.sol:AvalancheL1MiddlewareTes
...
contract MockVault is IVaultTokenized {
    ...
    function activeSharesAt(uint48 timestamp, bytes calldata) public view override returns (uint256) {
        ...
        return totalShares > 0 ? totalShares : 200; // Default to 200 if not explicitly se
    }
...
function test_curatorSharesAssignedToCurrentOwnerInsteadOfHistorical() public {
	...
	// Step 7: Verify curator shares assignmen
...
1. Distributor calls `setRewardsAmountForEpochs(1, 1, USDC, 100)`
   ...
   → 100 USDC is transferred into the contrac
...
```solidity
 function test_UptimeTruncationCausesRewardLoss() public {
        ...
        // Demonstrate how small time lost can have big impac
        ...
        uint256 remainder = totalUptime % elapsedEpochs; // 2 seconds los
...
### Operator can over allocate the same stake to unlimited nodes within one epoch causing weight inflation and reward thef
...
function getActiveNodesForEpoch(
    ...
    // Gather all nodes from the never-removed se
...
contract PoCMissingLockingRewards is AvalancheL1MiddlewareTest {
    ...
    Rewards          internal rewards;          // Rewards contract under tes
...
Logs:
  ...
  Removing Alice's unbacked nodes at the earliest possible momen
...
- Performs the same `_wasActiveAt()` calculation twice for each vaul
