...
**Description:** The program validates `vault_gkhan_account` only by checking tha
...
### Hard-coded `9` decimals for xBELO mint can mismatch underlying BELO min
...
- `!stake.claimed` checked both in `withdraw_clmm_position` function and `WithdrawCLMMPosition` struc
