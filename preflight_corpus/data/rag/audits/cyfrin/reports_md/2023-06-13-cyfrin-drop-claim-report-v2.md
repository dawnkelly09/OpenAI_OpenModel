...
```diff
  struct ClaimData {
        ...
        uint128 minFee; // Minimum ETH fee amoun
+        uint64 numClaimers;// Number of users who already claimed //@audit -> add this variable to ClaimData struc
    }
```
...
```solidity
  function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external override {
       ...
       //@audit -> this verifies if already claimed or no
