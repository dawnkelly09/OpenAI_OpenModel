...
+       // if there is only 1 trove open and that is being liquidated, preven
...
function test_transferPoints_BypassVestingViaPreclaim() external {
    ...
    // user 1 needs to wait 3 days to bypass LockedAllocation rever
    ...
    // length lock via TokenLocker to give voting weigh
}
```
...
// to prevent point transfers being used to bypass the maximum preclaim limi
...
function test_allocateNewEmissions_unallocatedTokensDecreasedButZeroAllocated() external {
    ...
    // verify unallocated supply reduced by weekly emission percen
...
function test_unfreeze_failToRemoveActiveVotes() external {
    ...
    assertEq(frozenAmount, 0); // 0 frozen amoun
    ...
    assertGt(frozenAmount, 0); // positive frozen amoun
...
### Some `TroveManager` debt emission rewards can be los
...
function test_claimReward_someTroveManagerDebtRewardsLost() external {
    ...
    //
    // some rewards were not distributed and are effectively los
    ...
    // refresh mock oracle to prevent frozen feed rever
    mockOracle.refresh();
    ...
    // user1 gets almost all the weekly emissions apar
    // from an amount that is los
...
### Easily bypass `setGuardian` proposal passing requiremen
...
returns (uint256 totalWeeklyWeight, uint256 receiverWeeklyWeight) {
    ...
    // if not zero, also output receiver weekly weigh
...
function getReceiverWeeklyEmissions(
    ...
    // if there was weekly weight, calculate the amoun
...
function test_withdrawWithPenalty_activeLockWithZeroLocked() external {
    ...
    assertEq(activeLockData[0].amount, 0); // 0 locked amoun
    ...
    assertEq(lockData[0].amount, 0); // 0 locked amoun
...
```diff
      if (lockAmount - penaltyOnAmount > remaining) {
          ...
          // add any dust to the penalty amoun
...
function test_redeemCollateral_noCollateralForRemainingAmount() external {
    ...
    // update price oracle response to prevent stale rever
    mockOracle.setResponse(mockOracle.roundId() + 1,
                           mockOracle.answer(),
...
function test_claimReward_smallAmountOfStabilityPoolRewardsLost() external {
    ...
    //
    // a small amount of rewards was not distributed and is effectively los
}
```
...
BorrowerOperations::_setMinNetDeb
...
```diff
// if users tried to withdraw as much as possible, then subtrac
// the "unfilled" net amount (not inc penalties) from the user inpu
...
function setPassingPct(uint256 pct) external returns (bool) {
    // enforce this function can only be called by this contrac
...
### `TokenLocker::freeze` always emits `LocksFrozen` event with 0 amoun
...
### Consolidate `10000`, `MAX_FEE_PCT` and `MAX_PCT` used in many contracts into one shared constan
...
### Consolidate `DECIMAL_PRECISION` used in three contracts into one shared constan
...
### Consolidate `SCALE_FACTOR` used in two contracts into one shared constan
...
### Consolidate `REWARD_DURATION` used in four contracts into one shared constan
...
// then change `createNewProposal` to call the new private function passing in the week inpu
...
### Re-order `TokenLocker::penaltyWithdrawalsEnabled` to save 1 storage slo
...
```solidity
// Rate at which the total lock weight decreases each week. The total decay rate may no
