...
function test_yieldPhase_supportedVaults_userCantWithdrawVaultAssets() external {
    // user1 deposits $1000 USDe into the main vaul
    ...
    pUSDe.startYieldPhase();
    // totalAssets() still returns same amount as it is overridden in pUSDeVaul
    assertEq(pUSDe.totalAssets(), user1AmountInMainVault);
    ...
    // create an additional supported ERC4626 vaul
    MockERC4626 newSupportedVault = new MockERC4626(USDe);
    pUSDe.addVault(address(newSupportedVault));
    // add eUSDe again since `startYieldPhase` removes i
    ...
    // user2 deposits $600 into each vaul
    ...
    // verify user2 entitled to withdraw their total token amoun
    ...
    // can't withdraw anymore, even trying 1 wei will rever
    vm.expectRevert();
...
function redeemRequiredBaseAssets (uint baseTokens) internal {
    for (uint i = 0; i < assetsArr.length; i++) {
        ...
        // @audit only withdraw if a single withdraw can satisfy desired amoun
...
* keep track of the total currently redeemed amoun
* calculate the remaining requested amount as the requested amount minus the total currently redeemed amoun
...
* if the current vault could redeem more than the remaining requested amount, redeem only enough to satisfy the remaining requested amoun

The above strategy ensures that:
* small amounts from multiple vaults can be used to fulfill the requested amoun
...
contract MockERC4626 is ERC4626 {
    ...
    function maxWithdraw(address owner) public view override returns (uint256) {
        ...
        if (hacked) {
            return super.maxWithdraw(owner) / 2; // Reduce max withdraw by half to simulate some limi
        }
...
function test_redeemRequiredBaseAssetsDoS() public {
    ...
    // deposit USDe from another accoun
    ...
    // as a result of a hack and imposition of a limi
    eUSDe.hack();
    ...
    // attempt to withdraw from eUSDe2 vault, but redeemRequiredBaseAssets() skips withdrawal attemp
...
### `MetaVault::addVault` should enforce identical underlying base asse
...
* `redeemRequiredBaseAssets` won't work as expected since the newly supported vault doesn't have the same base asse
...
function test_vaultSupportedWithDifferentUnderlyingAsset() external {
    // create ERC4626 vault with different underlying ERC20 asse
    MockUSDe differentERC20 = new MockUSDe();
    MockERC4626 newSupportedVault = new MockERC4626(differentERC20);

    // verify pUSDe doesn't have same underlying asset as new vaul
    ...
    // the newly supported vault doesn't have the same base asse
}
```
...
function test_supportedVaultsRemovedWhenYieldPhaseEnabled() external {
    ...
    // admin triggers yield phase on main vaul
    pUSDe.startYieldPhase();
...
function test_maxWithdraw_WhenWithdrawalsPaused() external {
    ...
    // attempting to withdraw would rever
...
function test_maxDeposit_WhenDepositsPaused() external {
    ...
    // attempting to deposit would rever
