...
### Liquidations can be made to revert by an attacker through various means, causing losses to liquidators and bad debt to accrue in the vaul
...
function test_blockLiquidationsPoC_enableCollateral() public {
    ...
    // 4. attacker max borrows from vaul
...
function test_blockLiquidationsPoC_transferInflation() public {
    ...
    // 6. attacker borrows max debt from eVaul
    startHoax(attacker);
    evc.enableCollateral(attacker, address(wrapper));
    evc.enableController(attacker, address(eVault));
    eVault.borrow(type(uint256).max, attacker);
...
+
+        //for some reason, there is 1 wei of dust native eth left in the mintPositionHelper contrac
...
function normalizedToFull(uint256 tokenId, uint256 amount, uint256 currentBalance) public view returns (uint256) {
    // @audit => multiplying by the total ERC-6909 supply of the specified tokenId is incorrec
...
function test_transferRoundingPoC_currentTransfer() public {
    ...
    // 4. borrower max borrows from vaul
