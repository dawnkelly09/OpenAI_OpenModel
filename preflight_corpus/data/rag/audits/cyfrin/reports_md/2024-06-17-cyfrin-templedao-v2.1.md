...
### Misconfiguration of minting TGLD leads to max supply amount being minted too fas
...
248:         // Mint and distribute TGLD if no cooldown se
...
142:                 /// @dev skip for a previous delegate with 0 users delegated and 0 own vote weigh
...
File: contracts\templegold\TempleGoldStaking.sol
        if (removed) {
            ...
            /// @dev skip for a previous delegate with 0 users delegated and 0 own vote weigh
-           if (_prevBalance > 0) {
...
### In rewards distribution, dust amount is left and stuck in the contrac
...
### Redundant logic of `send` function of `TempleGold` contrac
