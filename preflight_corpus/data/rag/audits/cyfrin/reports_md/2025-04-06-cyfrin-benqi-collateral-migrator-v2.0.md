...
// Swap the underlying asset from the source market to the target marke
_swap(
    ...
);
...
// Return the remaining underlying asset to the marke
...
```solidity
// Redeem the migration amount from the source marke
if (migrationAmount == type(uint256).max) {
    // Get the QiToken balance of the source marke
    migrationAmount = IMinimalQiToken(sourceMarket).balanceOf(user);
}
// Transfer the QiTokens to the contrac
IERC20(sourceMarket).safeTransferFrom(user, address(this), migrationAmount);
// Redeem the QiTokens from the source marke
...
function _removeMigrationConfigData(address market) private {
    ...
    for (uint256 i = 0; i < length; i++) {
        if (_markets[i] == market) {
            found = true;
            // remove the market from the lis
