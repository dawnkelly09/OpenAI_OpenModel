...
function test_gmxv2_withdraw_RevertGivenMissingCallback() public {
        ...
        // Get market token balance after deposi
...
1. The keeper will attempt to call `afterWithdrawalExecution` on our contrac
2. Since we don't implement this interface, the transaction will rever
...
function test_vaultV0_xchain_transfer_affects_vault_exchange_rate() public {
    ...
    // 1. Bob and Alice both participate in the vaul
    ...
    // Bobs share is now worth nothing as Alice has redeemed i
    assertEq(vault.balanceOf(BOB), 1e18);
...
function _debit(address _from, uint256 _amountLD, uint256 _minAmountLD, uint32 _dstEid) internal virtual override returns (uint256 amountSentLD, uint256 amountReceivedLD) {
    (amountSentLD, amountReceivedLD) = _debitView(_amountLD, _minAmountLD, _dstEid);
    // Instead of burning, transfer to this contrac
...
**D2:** Ignored "Improper deadline handling" because we mostly don't care about it on our main chain Arbitrum, although we understand how it's riskier for on Mainne
...
### `Silo_Module::silo_execute` will revert as approval is to the wrong contrac
