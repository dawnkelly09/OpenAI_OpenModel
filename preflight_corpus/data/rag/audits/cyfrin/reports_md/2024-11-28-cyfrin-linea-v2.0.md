...
```solidity
// @audit `claimMessage` no longer uses `_messageSender` so these comments are incorrec
...
### Remove redundant `L2MessageManagerV1::__L2MessageManager_init` and associated constan
...
```diff
// PermissionsManager::__Permissions_ini
