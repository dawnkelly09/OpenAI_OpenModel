...
+            nft, bondingCurve, assetRecipient, poolType, delta, fee, spotPrice, _idLis
+        );
+        return pair;
+    }
+
...
+                    // console.log("sender balance: %s", msg.sender.balance);
             ...
             // Otherwise, we need to do some partial fill calculations firs
@@ -488,10 +489,15 @@ contract VeryFastRouter {
         }

         // Send excess ETH back to token recipien
...
```solidity
        ...
        returns (address payable[] memory receivers, uint256[] memory amounts)
    {
        ...
        // work out the expected royalty paymen
...
351:                             priceToFillAt,//@audit-info min expected outpu
...
```
    function testSwapBinarySearch_audit() public{
        ...
        //2. setup a ERC721 ETH pair with property checker is zero address and zero deposit amoun
        pair = setUpPairERC721ForSale(0, address(0), nftIds);
        ...
        // with no slippage, inputAmount should exactly be equal to the last partialFillAmoun
        ...
        if (tokenAddress != address(0)) {
            ERC20(tokenAddress).approve(address(router), 1e18 ether); //-n give approval for very high amoun
...
### Partially fillable order could rever
**Description:**
...
### Event parameter names are incorrec
...
- 268:             // Deduct royalties from outputAmoun
+ 268:             // @notice Deduct royalties from outputAmoun
...
- 331:         // Otherwise, we return the recipient if it's been se
+ 331:         // @notice Otherwise, we return the recipient if it's been se
...
- 529:                 // update numRecipients to match new recipients lis
+ 529:                 // @notice update numRecipients to match new recipients lis

- 544:         // Ensure royalty total is at most 25% of the sale amoun
+ 544:         // @notice Ensure royalty total is at most 25% of the sale amoun
...
- 655:         // Prevent calling the pair's underlying nf
+ 655:         // @notice Prevent calling the pair's underlying nf
...
- 34:         // Transfer inputAmountExcludingRoyalty ETH to assetRecipient if it's been se
+ 34:         // Transfer inputAmountExcludingRoyalty ETH to assetRecipient if it has been se
...
- 671:         // transfer NFTs from caller to recipien
+ 671:         // Transfer NFTs from caller to recipien
...
- 55:                     // i.e., no need to rever
+ 55: /// i.e., no need to rever

- 57:                 // Otherwise, the callback had an error, and we should rever
+ 57: /// Otherwise, the callback had an error, and we should rever
...
Initializers could be front-run, allowing an attacker to either set their own values, take ownership of the contract, and in the best case forcing a re-deploymen
