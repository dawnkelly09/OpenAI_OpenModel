...
1. **Use token.decimals() instead of 18.**
   ...
   uint256 scale = 10 ** decimals;                      // token’s native uni
...
### Volume-based fee can be bypassed with a wrapper contrac
...
function _executeAction(address _wallet, Action memory _action) internal returns (uint256 feeInUSD) {
    ...
    if (exist) {
        ...
        uint256 volume = feeType == IFeeController.FeeType.Deposi
...
**Recommended Mitigation:** Calculate `beneficiaryAmount` and `creatorAmount` as above, then se
```solidity
