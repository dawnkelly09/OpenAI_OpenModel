...
### `Goldilend.lock()` will always rever
...
```solidity
  function repay(uint256 repayAmount, uint256 _userLoanId) external {
    ...
    poolSize += userLoan.interest * (1000 - (multisigShare + apdaoShare)) / 1000; //@audit should use interest instead of userLoan.interes
...
  }
```
...
```solidity
  function cancel(uint256 proposalId) external {
    ...
    if(GovLocks(govlocks).getPriorVotes(proposal.proposer, block.number - 1) > proposalThreshold) revert AboveThreshold(); //@audit incorrec
