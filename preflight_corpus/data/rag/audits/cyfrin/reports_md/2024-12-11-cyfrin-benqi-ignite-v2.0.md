...
```solidity
    ...
    if (qiRewardEligibilityByNodeId[nodeId]) {
        qiRedemptionAmount += validatorRewarder.claimRewards(
            ...
            registration.tokenDeposits.tokenAmoun
        );
    }
...
it("can have slippage outside of max and min", async function () {
    ...
    // max slippage is updated below i
...
// Transfer tokens from the user to the contrac
...
### Placeholder recipient constants in `Ignite` should be updated before deploymen
...
```diff
  /**
   * @notice Register a new node with a prevalidated QI deposit amoun
...
contract TestZeroAddressCall is Test {
    function test_zeroAddressCall() external {
        ...
        assertFalse(revertsAsExpected); // the call itself does not rever
