...
contract ERC721PowerAttack {
    ...
    // the power nfts that exist (to zero), regardless of who owns them, and sets the curren
...
### Under-funded eth distribution proposals can be created causing claiming rewards to rever
...
```javascrip
          it("attacker can bypass token sale maxAllocationPerUser to buy out the entire tier", async () => {
            ...
            // txn individually, doesn't factor in the total amoun
            // user has already bough
...
```javascrip
      it("audit micropool rewards short-change delegator for long proposals with multiple delegations", async () => {
        ...
        // DELEGATEE votes on the second proposal for the first time using the firs
...
function getNftPower(uint256 tokenId) public view override returns (uint256) {
    ...
    // Calculate the minimum possible power based on the collateral of the nf
...
### `DistributionProposal` 'for' voter rewards diluted by 'against' voters and missing rewards permanently stuck in `DistributionProposal` contrac
...
```javascrip
      it("audit static GovUserKeeper::_nftInfo.totalPowerInTokens in quorum denominator can incorrectly make it impossible to reach quorum", async () => {
        ...
        // create a new VOTER account and mint them the only power nf
        let VOTER = await accounts(10);
        await nftPower.safeMint(VOTER, 1);

        // switch to using a new ERC20 token for voting; lets us
        // control exactly who has what voting power without worrying abou
        ...
        // verify that the true totalPower has decremented to zero as the nf
...
* when users use power nfts to [vote personally, this uses the current power](https://github.com/dexe-network/DeXe-Protocol/blob/440b8b3534d58d16df781b402503be5a64d5d576/contracts/libs/gov/gov-user-keeper/GovUserKeeperView.sol#L185) of the power nf
* when users delegate power nfts and [have the delegatee vote, this caches the minimum power](https://github.com/dexe-network/DeXe-Protocol/blob/440b8b3534d58d16df781b402503be5a64d5d576/contracts/gov/user-keeper/GovUserKeeper.sol#L266) of the power nf
...
```javascrip
       it("audit actual power nft voting power doesn't match total nft voting power", async () => {
          ...
          // MASTER has the one & only power nf
...
```javascrip
        it("audit verified: nft totalPower > 0 when all nfts lost power incorrectly makes it impossible to reach quorum", async () => {
          ...
          // set newNftPower as the voting nf
          // need to comment out check preventing updating existing
          // nft address in GovUserKeeper::setERC721Address()
          await impersonate(govPool.address);
...
### Using fee-on-transfer tokens to fund distribution proposals creates under-funded proposals which causes claiming rewards to rever
...
```javascrip
      it("using fee-on-transfer tokens to fund distribution proposals prevents claiming rewards", async () => {
        ...
        // this token also doesn't implement approvals so don't need to worry about tha
        ...
        // contract than the inputted amoun
      ...
      describe("audit tiered revoteDelegate", () => {
        ...
        it("audit testing 3 layer revote delegation", async () => {
          ...
          assert.equal(
            ...
            "0" // nothing delegated to AUDITOR ye
          );
          assert.equal(
...
### Proposal creation uses incorrect `ERC721Power::totalPower` as nft power not updated before snapsho
...
- Validators think P is not in the best interest of DAO and vote agains
    ...
    function _getInitialVotingRewards(
        ...
        return
            coreRawVotes.ratio(core.settings.rewardsInfo.voteRewardsCoefficient, PRECISION).ratio(
                voteInfo.totalVoted,
                coreVotes
            ); //@audit -> initial rewards are calculated proportionate to the vote rewards coefficien
    }
```
