...
`apiData` - Contains parameters including the token to be swapped (tokenFrom) and the amoun
...
```solidity
/**
 * @title DelegationMetaSwapAdapterConsistencyExploitTes
...
contract DelegationMetaSwapAdapterConsistencyExploitTest is DelegationMetaSwapAdapterBaseTest {
    ...
    function test_tokenFromInconsistencyExploit() public {
        ...
        // - swapData references tokenB as tokenFrom, with a much larger amoun
        ...
        // - swapData actually references tokenB with a much larger amoun
        ...
        bytes memory maliciousApiData = _encodeExploitApiData(
            ...
            amountToTransfer,
            exploitSwapData // but use swapData that uses tokenB with large amoun
        );

        // Create a delegation from vault to subVaul
    ...
    // Helper to encode custom apiData for the exploi
    ...
    // Helper to encode custom swapData for the exploi
        ...
        returns (bytes memory)
    {
        ...
        return abi.encode(
            ...
            _amountTo,
            bytes(""), // Not important for the exploi
            uint256(0), // No fee
            address(0), // No fee walle
            false // No fee to
        );
    }
    ...
    function _setUpMockContractsWithCustomMetaSwap() internal {
        ...
        // give allowance of tokenB to the metaswap contrac
...
function swapByDelegation(bytes calldata _apiData, Delegation[] memory _delegations) external {
    ...
    // Decode swapData to extract the actual token and amoun
    ...
    // @audit Validate amounts are consisten
...
### Malicious user can create delegations that will spend unlimited amount of gas with `SpecificActionERC20TransferBatchEnforcer` cavea
...
```
Hence, calldata size can be arbitrarily increased without compromising execution outpu
...
function testGriefingAttackSpecificActionERC20TransferEnforcer() public {
        ...
        bytes memory terms = abi.encodePacked(
            ...
            address(0x1), // recipien
            uint256(100), // amoun
            address(0x1), // firstTarge
...
Output gas:
```
Ran 1 test for test/enforcers/SpecificActionERC20TransferBatchEnforcer.t.sol:SpecificActionERC20TransferBatchEnforcerTes
[PASS] testGriefingAttackSpecificActionERC20TransferEnforcer() (gas: 207109729)
```

**Recommended Mitigation:** Consider constraining the calldata size to a specific limi
        ...
        onlyDefaultExecutionMode(_mode)
    {
        ...
        if (
            executions_[1].target != terms_.tokenAddress || executions_[1].value != 0 || executions_[1].callData.length != 68
                ...
                || address(uint160(uint256(bytes32(executions_[1].callData[4:36])))) != terms_.recipien
                || uint256(bytes32(executions_[1].callData[36:68])) != terms_.amoun
        ) {
...
```diff
        ...
        onlyDefaultExecutionMode(_mode)
    {
        ...
        if (
--          executions_[0].target != terms_.firstTarget || executions_[0].value != 0
++          executions_[0].target != terms_.firstTarge
                || keccak256(executions_[0].callData) != keccak256(terms_.firstCalldata)
        ) {
            revert("SpecificActionERC20TransferBatchEnforcer:invalid-first-transaction");
        }
...
2. Avoid overriding storage data when spent is 0 and data is already se
```diff
    function _validateAndConsumeAllowance(
        bytes calldata _terms,
        bytes calldata _executionCallData,
