...
function notifySwap(PoolId id, address[] calldata lpTokens) external onlyAllowedHooks {
    // @gas cheaper not to cache length for calldata array inpu
    ...
    returns (uint128 liquidity)
{
    ...
    if (rangeLiquidity == 0) {
        ...
        // Calculate the amount of LP tokens to min
...
contract FullRangeHook is BaseHook, ReentrancyGuard, ITokenizedHook {
    ...
--  /// @notice Erorr raised if the tick spacing is not defaul
++  /// @notice Error raised if the tick spacing is not defaul
...
contract ValkyrieHooklet is IHooklet {
    ...
    /*
    This Hooklet needs the following flags in this contrac
...
--      IncentivizedPoolId indexed id, address indexed token, address indexed recepient, uint256 amoun
++      IncentivizedPoolId indexed id, address indexed token, address indexed recipient, uint256 amoun
    );
    ...
--  /// @notice Rewards checckpoints for an user
++  /// @notice Rewards checkpoints for an user
    struct RewardCheckpoint {
        /// @notice Timestamp of the checkpoin
        uint128 timestamp;
