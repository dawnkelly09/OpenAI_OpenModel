...
>>      bytes32, //@audit ignores UserOpHash from the Entry Poin
...
contract EIP7702EntryPointReplayAttackTest is BaseTest {
    ...
    // Implementation with the new EntryPoin
    EIP7702StatelessDeleGator newImpl;

    function setUp() public override {
        super.setUp();

        // Deploy a second EntryPoin
        newEntryPoint = new EntryPoint();
        vm.label(address(newEntryPoint), "New EntryPoint");

        // Deploy a new implementation connected to the new EntryPoin
    ...
    function test_replayAttackAcrossEntryPoints() public {
        // 1. Create a UserOp that will be valid with the original EntryPoin
        ...
        // Create the UserOp with current EntryPoin
        ...
        // Alice signs it with the current EntryPoint's contex
        ...
        // Execute the original UserOp through the first EntryPoin
        ...
        // 3. Attempt to replay the original UserOp through the new EntryPoin
        vm.prank(bundler);
...
```solidity
 function test_AllowedMethods_DuplicateMethodsGriefing() public {
        ...
        // Create delegation with allowed methods cavea
...
**Impact:** Users cannot use `AllowedCalldataEnforcer` to authorize delegations that should only permit simple ETH transfers to contracts with receive(). Common use cases like depositing ETH to WETH (which uses the receive() function) cannot be properly enforced through this cavea
...
>>                  if (delegation_.signature.length == 0) {
                        // Ensure that delegations without signatures rever
                    ...
                    if (delegation_.delegator.code.length == 0) {
                        ...
                        // Validate delegation if it's a contrac
                        ...
...
```solidity
    function isValidSignature(bytes32 _hash, bytes calldata signature) external view override returns (bytes4 magic) {
        ...
        // domainVerifier doesn't exist or the signature is invalid for the domain - fall back to the defaul
...
```solidity
   // NativeTokenPaymentEnforcer.sol
    function afterAllHook( ... ) ... {
        ...
        ...
        // Attempt to redeem the delegation and make the paymen
