...
* Make `RewardsDistributor::triggerRoot` a permissioned function such that an attacker can't call i
...
function _depositLPIncentive(
    ...
    // the actual amount received after the transfer fee was deducted in-transi
...
* read the `before` transfer token balance of `RewardsDistributor` contrac
* perform the token transfer
* read the `after` transfer token balance of `RewardsDistributor` contrac
* calculate the difference between the `after` and `before` balances to get the true amount that was received by the `RewardsDistributor` contract accounting for the fee that was deducted in-transi
...
* adding themselves to the blacklis
...
### Refactor `zeroRoot` declared in multiple functions into a private constan
