...
* no access control meaning anyone can call i
...
contract StealMargin_Integration_Test is Base_Test {
    ...
    function test_AttackerStealsUserCollateral() external {
        // @audit naruto the victim will create an account and deposi
        ...
        emit TradingAccountBranch.LogDepositMargin(
            users.naruto, victimTradingAccountId, address(usdToken), amountToDeposi
        );

        // it should transfer the amount from the sender to the trading accoun
        ...
        // @audit sasuke the attacker will steal naruto's deposi
        // beginning state: theft has not occured
        assertEq(0, usdToken.balanceOf(users.sasuke));
        //
        // @audit 1) sasuke creates their own hacked `AccountNFT` contrac
        ...
        // hacked `AccountNFT` contract the official contrac
        ...
        // contract to steal ownership of the victim's accoun
...
function test_ImpossibleToLiquidateAccountWithMultipleMarkets() external {
    ...
    // then user would only have 1 active marke
    //
...
function test_AttackerMintsFreeUSDzOpenThenQuicklyClosePositionMarketNegMakerFee() external {
    ...
    // This allows attackers to mint free USDz withou
    ...
    // naruto opens position in BTC marke
    openManualPosition(BTC_USD_MARKET_ID, BTC_USD_STREAM_ID, MOCK_BTC_USD_PRICE, tradingAccountId, USER_POS_SIZE_DELTA);
...
// forge test --match-contract SetTes
...
### Trader can't reduce open position size when under initial margin requirement but over maintenance margin requiremen
...
function test_TraderCantReducePositionSizeWhenCollateralUnderIntitialRequired() external {
    ...
    // naruto's position now is below the initial margin requiremen
...
### Users can easily bypass collateral `depositCap` limit using multiple deposits under the limi
...
function test_ImpossibleToClosePositionIfMarkedDisabledButStillLiquidatable() external {
    ...
    // protocol operator disables the BTC marke
    ...
    // naruto attmpts to close their opened leverage BTC position but i
...
* for the incorrect old BTC marke
...
function test_KeeperFillsOrderToIncorrectMarketAfterUserUpdatesOpenOrder() external {
    ...
    // naruto creates an open order in the BTC marke
    perpsEngine.createMarketOrder(
        OrderBranch.CreateMarketOrderParams({
            tradingAccountId: tradingAccountId,
    ...
    // 2) naruto updates their open order to place it on ETH marke
    ...
    // load naruto's position for BTC marke
...
### `TradingAccount::getAccountMarginRequirementUsdAndUnrealizedPnlUsd` uses incorrect price during order settlemen
...
function test_ImpossibleToClosePositionIfSettlementDisabledButStillLiquidatable() external {
    ...
    // protocol operator disables settlement for the BTC marke
    changePrank({ msgSender: users.owner });
...
function test_configChangeRetrospectivelyImpactsAccruedFundingRates() external {
    ...
    // snapshot EVM state at this poin
...
180:         settlementFeeRecipient: globalConfiguration.settlementFeeRecipien
181:     }),
...
### Gracefully handle state where perp market `maxOpenInterest` is updated to be smaller than the current open interes
...
### Inconsistent validation while creating/updating a perp marke
...
function test_LiquidationRevertsWhenPriceFeedReturnsZero() external {
    ...
    // as recently happened during the Linea hack such tha
...
contract GasMeter {
    ...
    function meterStaticCall(
        ...
        // This code will never be executed, because huffGasMeter() will return or rever
...
function checkLiquidatableAccounts() returns (uint128[] memory liquidatableAccountsIds) {
{
    ...
    // @audit only load global config if we didn't fail fas
    GlobalConfiguration.Data storage globalConfiguration = GlobalConfiguration.load();
...
```solidity
// if account is not liquidatable, skip to next accoun
...
UD60x18 liquidatedCollateralUsdX18 = tradingAccount.deductAccountMargin({
    feeRecipients: FeeRecipients.Data({
        ...
        settlementFeeRecipient: liquidationFeeRecipien
    }),
...
// revert if newOpenInterest > maxOpenInteres
...
// calculate new skew if txn didn't rever
...
if(shouldCheckMaxSkew && newSkew.abs().gt(ud60x18(self.configuration.maxSkew).intoSD59x18()) {
    // determine whether skew is being reduced or no
