...
```solidity
    function test_PayOut() public {
        ...
        performUpkeep();
        // Fulfill request for payout  with PayOutamoun
...
```solidity
    function test_FrontrunCreateOffer() public {
        ...
        // alice allows minimal amount of USDC to the contrac
        vm.startPrank(alice);
...
```solidity
TunnlTwitterOffers.sol - sendFunctionsReques
395:         s_offers[offerId].status = requestType == RequestType.Verification
396:             ? Status.VerificationInFligh
...
### Unnecessary use of OpenZeppelin's nonReentran
