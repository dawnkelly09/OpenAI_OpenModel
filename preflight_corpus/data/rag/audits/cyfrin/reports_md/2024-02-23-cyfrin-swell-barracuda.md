...
// function only callable by admin & bo
...
### `swExit::getProcessedRateForTokenId` returns `true` with valid `processedRate` for non-existent `tokenId` inpu
...
```typescrip
    it("Should revert updating operator controlling address to existing address", async () => {
      ...
      // the same as the newly created operator - should revert but doesn'
...
// that storage locations were just updated from to eliminate redundant bu
...
// run from base project directory with:
// halmos --function test --match-contract ParseTes
...
```solidity
  ...
  function handleReprice(
    UpgradeableRepriceSnapshot calldata _snapsho
  ) internal {
    // only call getSnapshotStruct() once
    UpgradeableRepriceSnapsho
...
### Use constant for unchanging deposit amoun
