...
### `MembershipERC1155` profit tokens can be drained due to missing `lastProfit` synchronization when minting and claiming profi
...
it("lets users steal steal account balance by transferring tokens and claiming profit", async function () {
    ...
    // user creates a second account and transfers their tokens to i
    ...
    // second account can claim profi
    ...
    // contract balance has decreased with twice the profi
...
it("lets users steal steal account balance by minting after profit is sent", async function () {
    ...
    // new user mints a token after profit and can claim first users profi
...
it("lets users front-run profit distribution", async function () {
  ...
  // this can be done with a deployed contrac
  for(let i = 0; i < 9; i++) {
...
**One World Project:** This is a business logic requirement. We have to allow upgradation even after the tier is full. So, the total minted will remain how many were minted, but the upgraded members will be above and beyond tha
...
function mint(address to, uint256 tokenId, uint256 amount) external override onlyRole(OWP_FACTORY_ROLE) {
    ...
    totalSupply += amount * 2 ** (6 - tokenId); // Update total supply with weigh
}
```
