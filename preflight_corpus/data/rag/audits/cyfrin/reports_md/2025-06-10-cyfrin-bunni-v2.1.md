...
function test_ForkHookHandleSwapDrainRawBalancePoC() public {
    ...
    address USDCVault = 0x3b028b4b6c567eF5f8Ca1144Da4FbaA0D973F228; // Euler vaul
    ...
    uint256 initialToken0Deposit = 10_000e6; // Using a big amount in order to not execute too many reentrancy iterations, but it works with whatever amoun
    ...
    console.log(
        "Stolen USDC",
        poolManager.balanceOf(customHook, Currency.wrap(address(token0)).toId()) - initialToken0Deposi
    );
}
```
...
Output:
```bash
Ran 1 test for test/BunniHub.t.sol:BunniHubTes
...
```solidity
minRawTokenRatio0: 0,           // set to 0% in order to have all deposited funds accounted into the vaul
targetRawTokenRatio0: 0,        // set to 0% in order to have all deposited funds accounted into the vaul
maxRawTokenRatio0: 0,           // set to 0% in order to have all deposited funds accounted into the vaul
```
...
CustomHook(payable(customHook)).initiateAttack(
    ...
    sharesToMint,
    Vector.HOOK_HANDLE_SWAP,
    ERC4626(USDCVault).balanceOf(address(hub)) / sharesToMin
);
...
if (address(state.vault0) != address(0) && reserveAmount0 != 0) {
    ...
    // add amount spent on vault deposit to the total amount spen
...
function test_PoCVaultDoS() public {
    ...
    // Set up conditions
    // 1. Ensure that raw balance is greater than the max, hence it would need to trigger the vaul
    // deposi
...
function isValidParams(int24 tickSpacing, uint24 twapSecondsAgo, bytes32 ldfParams) internal pure returns (bool) {
    uint8 shiftMode = uint8(bytes1(ldfParams)); // use uint8 since we don't know if the value is in range ye
...
@>              (int256 reserveChange, int256 rawBalanceChange) = _updateVaultReserveViaClaimTokens(
                    (outputAmount - outputRawBalance).toInt256(), outputToken, outputVaul
                );
                zeroForOne
...
@>          self[indexUpdated] = intermediateUpdated;
        ...
        returns (Observation memory)
    {
        unchecked {
            ...
            // if the current tick moves more than the max abs tick movemen
...
### am-AMM fees could be incorrectly used by rebalance mechanism as order inpu
...
```solidity
    function rebalanceOrderPreHook(RebalanceOrderHookArgs calldata hookArgs) external override nonReentrant {
        ...
        // this is used to compute the order output amoun
...
(2)>        : hookFeesBaseSwapFee;
        ...
        if (exactIn) {
            ...
            // swap fee is taken by decreasing the output amoun
...
```solidity
            ...
            // compute the swap fee and the hook fee (i.e. protocol fee)
            // swap fee is taken by increasing the input amoun
...
@>              hookFeesAmount = inputAmount.mulDivUp(hookFeesBaseSwapFee, SWAP_FEE_BASE - hookFeesBaseSwapFee).mulDivUp(
            ...
            // set the am-AMM fee to be the swap fee amoun
            // don't need to check if am-AMM is enabled since if it isn'
            // BunniHook.beforeSwap() simply ignores the returned values
            // this saves gas by avoiding an if statemen
...
contract RebalanceWithBunniLiqTest is BaseTest {
    ...
    function test_outputExcessiveBidTokensDuringRebalanceAndRefund() public {
        ...
        bunniHook.bid(
            poolKey1.toId(), address(alice), bytes6(abi.encodePacked(uint24(1e3), uint24(2e3))), minRent, bidAmoun
        );
        ...
        // Record logs to capture the OrderEtched even
        vm.recordLogs();
        swapper.swap(poolKey2, params, type(uint256).max, 0);

        // Find the OrderEtched even
        Vm.Log[] memory logs_ = vm.getRecordedLogs();
        Vm.Log memory orderEtchedLog;
        for (uint256 i = 0; i < logs_.length; i++) {
            if (logs_[i].emitter == address(floodPlain) && logs_[i].topics[0] == IOnChainOrders.OrderEtched.selector) {
        ...
        // Decode the order from the even
        IFloodPlain.SignedOrder memory signedOrder = abi.decode(orderEtchedLog.data, (IFloodPlain.SignedOrder));
...
contract BunniTokenTest is BaseTest, IUnlockCallback {
    ...
    function test_PoCCrossContractReentrantHooklet() external {
        ...
        // update state in User contract to use refB in deposi
        referrer = refB;
...
### Potential erroneous surging when vault token decimals differ from the underlying asse
...
// take in max(amountSpecified, inputAmount) such that if amountSpecified is greater we just happily accept i
...
function test_transferFromPoC() external {
    ...
    // zero transfer from bob to alice doesn't rever
    vm.prank(alice);
...
### `BuyTheDipGeometricDistribution` parameter encoding documentation is inconsisten
...
```solidity
    if (params.useQueuedWithdrawal) {
        ...
        // need to withdraw the full queued amoun
