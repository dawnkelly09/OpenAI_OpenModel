...
```javascrip
  it('Cyfrin: off-chain mechanism in an incorrect order can lead to user funds being stolen', async () => {
    ...
    // at this point user has 1000 ether staked and 9000 ether in his walle
    assert.equal(fromEther(await token.balanceOf(accounts[1])), 9000)
    assert.equal(fromEther(await stakingPool.balanceOf(accounts[1])), 1000)
    ...
    // at this point user has 1500 ether staked and 9000 ether in his walle
    assert.equal(fromEther(await token.balanceOf(accounts[1])), 9000)
...
### User's funds are locked temporarily in the PriorityPool contrac
...
function _withdraw(address _account, uint256 _amount) internal {
    ...
    uint256 toWithdrawFromQueue = _amount <= totalQueued ? _amount : totalQueued;//@audit-info if the queue is not empty, we use that firs
    ...
    if (toWithdrawFromPool != 0) {
        stakingPool.withdraw(address(this), address(this), toWithdrawFromPool);//@audit-info withdraw from pool into this contrac
    }
...
**Impact:** User's LSD tokens can be locked temporarily in the PriorityPool contrac
...
```javascrip
 it('Cyfrin: user funds can be locked temporarily', async () => {
    ...
    // NOTE: at this point 500 ethers of LSD tokens are locked in the queue contrac
    ...
    // user's LSD tokens are still locked in the queue contrac
    await stakingPool.connect(signers[1]).approve(sq.address, toEther(500))
    await sq.connect(signers[1]).withdraw(toEther(500))
