...
### InfraVault's Permissionless Reward Claiming Can Allow Anyone to Lock Rewards in the MetaVaul
...
- The same tokens are simultaneously re-staked in the Infrared vaul
        ...
        if (_rewards[i].amount > 0) {
            if (_rewards[i].token == UNDERLYING_TOKEN()) {
                ...
                factory.depositIntoDolomiteMargin(
                    ...
                    _rewards[i].amoun
                ); //@audit restakes the reward amoun
...
```typescrip
    it('should demonstrate that iBGT rewards are re-staked after exit', async () => {
      ...
      // Get   the initial staking balance before exi
...
- In a future version, assume a user has accrued rewards in a NATIVE reward vaul
...
- `_getReward(_asset)` is called but retrieves from INFRARED vault instead of NATIVE vaul
...
```typescrip
  it('should revert when staking is paused and rewards are in iBGT', async () => {
      ...
      // Pause staking in the InfraredVaul
...
function _handleRewards(IInfraredVault.UserReward[] memory _rewards) internal {
    IIsolationModeVaultFactory factory = IIsolationModeVaultFactory(VAULT_FACTORY());
    for (uint256 i = 0; i < _rewards.length; ++i) {
        if (_rewards[i].amount > 0) {  // <-- This check is redundan
            if (_rewards[i].token == UNDERLYING_TOKEN()) {
                _setIsDepositSourceThisVault(true);
                factory.depositIntoDolomiteMargin(
                    DEFAULT_ACCOUNT_NUMBER,
...
- `_POLIsolationModeTraderBaseV2__initialize`:  No event when vault factory is se

_4. MetaVaultRewardTokenFactory_
- `depositIntoDolomiteMarginFromMetaVault`: No event for deposit from meta vaul
- `depositIntoDolomiteMarginFromMetaVault`: No event for deposit of other token from meta vaul
