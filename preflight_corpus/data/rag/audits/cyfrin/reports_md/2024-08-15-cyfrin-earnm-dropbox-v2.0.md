...
// still revert here since remainingBoxesAmountCache == 0 bu
...
```solidity
// Require that the user sends a fee of "claimFee" amoun
...
```solidity
// [Safety check] In case the amount to claim is 0, rever
...
// ---------------------------------------------------------------------------------------------
// Get the balance of Earnm ERC20 tokens of the contrac
...
// @audit remove `amountToClaim > 0` due to prev check which enforced tha
...
### Simplify checks to fail faster and remove negation operator on final boolean resul
...
```solidity
  function _determineTier(
    ...
    returns (uint128 /* tierId, function returns before the end of the execution */ )
  {
    // if the randomNumber is smaller than TIER_1_MAX_BOXES, first attemp
...
```solidity
    ...
    internal
  {
    ...
    for (uint32 i; i < boxAmount; i++) {
      ...
      // Mint the box; explicitly not using `_safeMint` to preven
      // re-entrancy issues
...
```solidity
    ...
    // [Safety check] Validate that there are still balance of boxes to min
    if (remainingBoxesAmountCache == 0) revert NoMoreBoxesToMint();

    // [Safety check] Validate that the remaining boxes to mint are greater or equal to the box amoun
...
```solidity
    ...
    internal
  {
    ...
    // [Gas] Cache mintedTierAmount and update cache during the loop to preven
  ...
  ERC721C
{
  ...
  uint32 internal remainingBoxesAmount; // Remaining boxes amount to min
