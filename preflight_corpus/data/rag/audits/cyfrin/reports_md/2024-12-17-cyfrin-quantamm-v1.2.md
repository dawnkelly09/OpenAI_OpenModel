...
```solidity
// if both tokens are within the first storage elemen
...
if (tokenIndex > 4) { // @audit should be >=
    // get the index in the second storage elemen
    index = tokenIndex - 4;
    targetWrappedToken = _normalizedSecondFourWeights;
...
if (tokenIndex > 4) {
    //get the index in the second storage in
    index = tokenIndex - 4;
    targetWrappedToken = _normalizedSecondFourWeights;
    tokenIndexInPacked -= 4;
} else {
...
require(
    _firstInt <= MAX32 &&
        ...
        _secondInt >= MIN32 &&
        _thirdInt <= MAX32 &&
        _firstInt >= MIN32 &&  // @audit should be _thirdIn
        _fourthInt <= MAX32 &&
        _firstInt >= MIN32 &&  // @audit should be _fourthIn
        _fifthInt <= MAX32 &&
        _firstInt >= MIN32 &&  // @audit should be _fifthIn
        _sixthInt <= MAX32 &&
        _firstInt >= MIN32 &&  // @audit should be _sixthIn
        _seventhInt <= MAX32 &&
        _firstInt >= MIN32 &&  // @audit should be _seventhIn
        _eighthInt <= MAX32 &&
        _firstInt >= MIN32,    // @audit should be _eighthIn
    "Overflow"
);
```
...
### Unauthorized role grants prevent `QuantAMMBaseAdministration` deploymen
...
//next expected update + time beyond tha
