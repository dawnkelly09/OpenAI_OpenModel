...
contract ConLiqWBTCUSDCTest is Test {
    ...
    function test_AttackerDrainsProtocolViaSetPositionWidth() public {
        ...
        // This is the transaction that the attacker sandwiches. The reason is tha
    ...
    function test_AttackerDrainsProtocolViaUnpause() public {
        ...
        // owner decides to unpause contrac
        //
...
* the caller as a reward for initiating the harves
* beefy protocol
...
contract ConLiqWBTCUSDCTest is Test {
    ...
    function setUp() public {
        ...
        // identified bug, for some reason (possibly block forking) the first deposi
    ...
    function test_StrategyAccumulatesNativeFeeTokensDueToRounding() public {
        ...
        // * this contract as the caller of the harves
        // * beefy
        // * the strategist registered with the strategy
        skip(10 hours);
        ...
        // collected fees should remain in strategy contrac
        ...
        // and being permanently stuck in the strategy contrac
        assertEq(IERC20(native).balanceOf(address(strategy)), 0);
    }
...
6. Owner calls `BeefyVaultConcLiq::withdraw` to receive many more tokens than they should be able to due to the inflated share count they received from the deposi
...
contract ConLiqWBTCUSDCTest is Test {
    ...
    function test_FirstDepositorCanInflateTheirShares() public {
        ...
        // to massively inflate their share coun
        withdraw(attacker, 0);
...
// forge test --match-contract PriceTes
...
```solidity
encountered 1 failing test in test/PriceTest.t.sol:PriceTes
...
```
Ran 1 test for test/PriceTest.t.sol:PriceTes
...
### Consider enforcing a min TWAP interval in `StrategyPassiveManagerUniswap::setTwapInterval` to avoid dangerous assignmen
...
// ideally `_onlyVault()` would return the vaul
