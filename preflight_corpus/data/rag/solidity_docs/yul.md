...
Solidity. The compiler uses Yul as an intermediate language in the
IR-based code generator (\"new codegen\" or \"IR-based codegen\"). Yul
is a good target for high-level optimisation stages that can benefit all
...
complexity of the stack itself. The programmer or auditor should no
...
allows specializing Yul to the requirements of different targe
...
``` yul
{
    function power(base, exponent) -> resul
    {
        switch exponen
        case 0 { result := 1 }
        case 1 { result := base }
...
`Yul object notation <yul-object>`{.interpreted-text role="ref"} so tha
...
and for the `standard-json interface <compiler-api>`{.interpreted-tex
role="ref"}:
...
- ASCII strings (e.g. `"abc"`), which may contain hex escapes `\xNN` and
  Unicode escapes `\uNNNN` where `N` are hexadecimal digits.
...
- An ASCII string is first viewed as a byte sequence, by viewing a
  ...
  sequence must not exceed 32 bytes. The byte sequence is padded with
  zeros on the right to reach 32 bytes in length; in other words, the
  string is stored left-aligned. The padded byte sequence represents a
  256-bit word whose most significant 8 bits are the ones from the firs
  byte, i.e. the bytes are interpreted in big endian form.
- A hex string is first viewed as a byte sequence, by viewing each pair
  of contiguous hex digits as a byte. The byte sequence must not exceed
  32 bytes (i.e. 64 hex digits), and is treated as above.
...
Both built-in and user-defined functions (see below) can be called in
the same way as shown in the previous example. If the function returns a
single value, it can be directly used inside an expression again. If i
...
from right to left to obtain the opcodes. In the case of the second line
...
If the declared variable should have a type different from the defaul
type, you denote that following a colon. You can also declare multiple
variables in one statement when you assign from a function call tha
...
For this, the number and types of the values have to match. If you wan
...
The `break` and `continue` statements can be used in the body to exi
...
Yul allows the definition of functions. These should not be confused
...
like the `return` statement in other languages just that it does no
...
Note that the EVM dialect has a built-in function called `return` tha
quits the full execution context (internal message call) and not jus
...
E(G, L, <fname(arg1, ..., argn)>: FunctionCall) =
    ...
    Let L' be a new local state such tha
...
currently selected version of the EVM. The only type available in this
dialect is `u256`, the 256-bit native type of the Ethereum Virtual
Machine. Since it is the default type of this dialect, it can be
omitted.
...
exactly one value. Opcodes marked with `F`, `H`, `B`, `C`, `I`, `L`, `P`
and `N` are present since Frontier, Homestead, Byzantium,
Constantinople, Istanbul, London, Paris or Cancun respectively.

In the following, `mem[a...b)` signifies the bytes of memory starting a
position `a` up to but not including position `b`, `storage[p]`
signifies the storage contents at slot `p`, and similarly,
`transientStorage[p]` signifies the transient storage contents at slo
`p`.

Since Yul manages local variables and control-flow, opcodes tha
...
`push` instructions.
  ...
  smod(x, y)                            F   x % y, for signed numbers in two\'s complement, 0 if
                                            y == 0
  ...
  signextend(i, x)                      F   sign extend from (i\*8+7)th bit counting from leas
                                            significan
  ...
  tstore(p, v)        [-]{.title-ref}   N   transientStorage\[p\] := v

  msize()                               F   size of memory, i.e. largest accessed memory index

  gas()                                 F   gas still available to execution

  address()                             F   address of the current contract / execution contex
  ...
  calldatacopy(t, f,  [-]{.title-ref}   F   copy s bytes from calldata at position f to mem a
  s)                                        position

  codesize()                            F   size of the code of the current contract / execution
                                            contex

  codecopy(t, f, s)   [-]{.title-ref}   F   copy s bytes from code at position f to mem a
                                            position
  ...
  returndatacopy(t,   [-]{.title-ref}   B   copy s bytes from returndata at position f to mem a
  f, s)                                     position

  mcopy(t, f, s)      [-]{.title-ref}   N   copy s bytes from mem at position f to mem a
                                            position
  ...
  create2(v, p, n, s)                   C   create new contract with code mem\[p\...(p+n)) a
  ...
  call(g, a, v, in,                     F   call contract at address a with inpu
  ...
  outsize)                                  output area mem\[out\...(out+outsize)) returning 0 on
                                            ...
                                            `See more <yul-call-return-area>`{.interpreted-tex
                                            role="ref"}
  ...
  outsize)                                  `See more <yul-call-return-area>`{.interpreted-tex
                                            role="ref"}
  ...
  in, insize, out,                          `call(g, a, 0, in, insize, out, outsize)` but do no
  ...
  selfdestruct(a)     [-]{.title-ref}   F   end execution, destroy current contract and send
                                            funds to a (deprecated)

  invalid()           [-]{.title-ref}   F   end execution with invalid instruction

  log0(p, s)          [-]{.title-ref}   F   log data mem\[p\...(p+s))

  log1(p, s, t1)      [-]{.title-ref}   F   log data mem\[p\...(p+s)) with topic t1

  log2(p, s, t1, t2)  [-]{.title-ref}   F   log data mem\[p\...(p+s)) with topics t1, t2

  log3(p, s, t1, t2,  [-]{.title-ref}   F   log data mem\[p\...(p+s)) with topics t1, t2, t3
  t3)

  log4(p, s, t1, t2,  [-]{.title-ref}   F   log data mem\[p\...(p+s)) with topics t1, t2, t3, t4
  t3, t4)
  ...
  blobbasefee()                         N   current block\'s blob base fee (EIP-7516 and
                                            EIP-4844)
  ...
  blobhash(i)                           N   versioned hash of transaction\'s i-th blob, 0 if blob
                                            does not exis
  ...
  prevrandao()                          P   randomness provided by the beacon chain (see note
                                            below)
...
This area is written to depending on how many bytes the called contrac
...
opcode. If it returns less data, then the remaining bytes are no
...
previously called `difficulty` have been changed and the instruction was
renamed to `prevrandao`. It can now return arbitrary values in the full
256-bit range, whereas the highest recorded difficulty value within
Ethash was \~54 bits. This change is described in
[EIP-4399](https://eips.ethereum.org/EIPS/eip-4399). Please note tha
irrelevant to which EVM version is selected in the compiler, the
semantics of instructions depend on the final chain of deployment.
::::
...
opcode will eventually undergo breaking changes in behavior as stated in
...
area, respectively. For the EVM, the `datacopy` function is equivalen
to `codecopy`.
...
See
`Using the Commandline Compiler <commandline-compiler>`{.interpreted-tex
...
- `n` is a decimal between 0 and 99 that specifies the number of inpu
  stack slots / variables
- `m` is a decimal between 0 and 99 that specifies the number of outpu
  stack slots / variables
- `data` is a string literal that contains the sequence of bytes

If you for example want to define a function that multiplies the inpu
...
might directly reuse result of the `calldataload` opcode, though)
...
restrictions can result in undefined behavior.

- Control-flow should not jump into or out of verbatim blocks, but i
  can jump within the same verbatim block. In particular, reverting or
  returning from the block is *not* allowed.
- Stack contents apart from the input and output parameters should no
  be accessed.
...
The optimizer does not analyze verbatim bytecode and always assumes tha
it modifies all aspects of state and thus can only do very few
optimizations across `verbatim` function calls.

The optimizer treats verbatim bytecode as an opaque block of code. I
...
code by the Yul optimizer. The only consequence of this is a differen
...
Data objects or sub-objects whose names contain a `.` can be defined bu
...
// The current object, sub-objects and data items inside the current objec
// are in scope.
object "Contract1" {
...
`expected number of contract executions <optimizer-parameter-runs>`{.interpreted-tex
...
Detailed information regarding the optimization sequence as well as a
list of abbreviations is available in the
`optimizer docs <optimizer-steps>`{.interpreted-text role="ref"}.
