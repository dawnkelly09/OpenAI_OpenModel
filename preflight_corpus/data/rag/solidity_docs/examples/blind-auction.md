...
The general idea of the following simple auction contract is tha
everyone can send their bids during a bidding period. The bids already
include sending some compensation, e.g. Ether, in order to bind the
bidders to their bid. If the highest bid is raised, the previous highes
bidder gets their Ether back. After the end of the bidding period, the
contract has to be called manually for the beneficiary to receive their
Ether - contracts cannot activate themselves.
...
contract SimpleAuction {
    ...
    function bid() external payable {
        ...
        // Ether back (the revert statemen
    ...
    function withdraw() external returns (bool) {
        ...
        if (amount > 0) {
            // It is important to set this to zero because the recipien
    ...
    function auctionEnd() external {
        // It is a good guideline to structure functions that interac
...
Another challenge is how to make the auction **binding and blind** a
the same time: The only way to prevent the bidder from just not sending
the Ether after they won the auction is to make them send it together
with the bid. Since value transfers cannot be blinded in Ethereum,
anyone can see the value.

The following contract solves this problem by accepting any value tha
...
contract BlindAuction {
    ...
    /// not the exact amount are ways to hide the real bid bu
    ...
    function withdraw() external {
        ...
        if (amount > 0) {
            // It is important to set this to zero because the recipien
            // can call this function again as part of the receiving call
            // before `transfer` returns (see the remark above abou
    ...
    // This is an "internal" function which means that i
