...
Alice and Bob use signatures to authorize transactions, which is
...
> 2.  Alice authorizes a payment by signing a message with her private
>     key.
...
nonce, which is the number of transactions sent by an account. The smar
...
the contract. Later, they decide to deploy the `RecipientPays` smar
...
first two lines of the `claimPayment()` function of the full contract a
the end of this section.

Furthermore, instead of destroying the contract by calling
`selfdestruct`, which is currently deprecated, we will disable the
contract\'s functionalities by freezing it, resulting in the reversion
of any call after it being frozen.
...
provides a function called `soliditySHA3` that mimics the behavior of
...
function
`ecrecover <mathematical-and-cryptographic-functions>`{.interpreted-tex
...
contract ReceiverPays is Freezable {
        ...
        notFrozen
    {
        ...
        // this recreates the message that was signed on the clien
...
Alice now builds a simple but complete implementation of a paymen
...
unidirectional payment channel between two parties (Alice and Bob). I
...
Bob is guaranteed to receive his funds because the smart contrac
escrows the Ether and honours a valid signed message. The smart contrac
...
to the participants in a payment channel to decide how long to keep i
...
maximum duration for the channel to exist. This is the `constructor` in
the `SimplePaymentChannel` contract, at the end of this section.
...
message intended for one payment channel from being used for a differen
channel.
...
When Bob is ready to receive his funds, it is time to close the paymen
channel by calling a `close` function on the smart contract. Closing the
channel pays the recipient the Ether they are owed and deactivates the
contract by freezing it, sending any remaining Ether back to Alice. To
...
`isValidSignature` and `recoverSigner` work just like their JavaScrip
...
Ether, and the sender is sent the remaining funds via a `transfer`. You
can see the `close` function in the full contract.
...
Unlike in the previous section, messages in a payment channel aren\'
...
>     escrowed.
> 4.  Verify that the signature is valid and comes from the paymen
>     channel sender.
