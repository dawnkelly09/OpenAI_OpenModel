...
form is named the Ethereum Natural Language Specification Forma
(NatSpec).
...
Doxygen-style comments and tags, there is no intention to keep stric
...
NatSpec includes the formatting for comments that the smart contrac
...
`function`, `enum`, `enum` value and `event` using the Doxygen notation
format. A `public` state variable is equivalent to a `function` for the
purposes of NatSpec.
...
- For Vyper, use `"""` indented to the inner contents with bare
  ...
  documentation](https://docs.vyperlang.org/en/latest/natspec.html).
...
used then the Solidity compiler will interpret a `///` or `/**` commen
...
`kind` must be one of `user` or `dev`. In the future it is possible tha
...
canonical signature as defined in the `Contrac
ABI <abi_function_selector>`{.interpreted-text role="ref"} and no
