...
::: index
! wei, ! finney, ! szabo, ! gwei, ! ether, ! denomination;ether
:::
...
seconds](https://en.wikipedia.org/wiki/Leap_second). Due to the fac
...
These suffixes cannot be applied to variables. For example, if you wan
...
::: index
abi, block, coinbase, difficulty, prevrandao, encode, number,
block;number, timestamp, block;timestamp, block;basefee,
block;blobbasefee, msg, data, gas, sender, value, gas price, origin
:::
...
values are provided by the EVM implementation that executes the contrac
...
if they did not receive any compensation, e.g. Ether.
...
- `abi.encodeWithSelector(bytes4 selector, ...) returns (bytes memory)`:
  ABI-encodes the given arguments starting from the second and prepends
...
See the documentation about the `ABI <ABI>`{.interpreted-tex
role="ref"} and the
`tightly packed encoding <abi_packed_mode>`{.interpreted-tex
...
- `bytes.concat(...) returns (bytes memory)`:
  `Concatenates variable number of bytes and bytes1, ..., bytes32 arguments to one byte array<bytes-concat>`{.interpreted-tex
  role="ref"}
...
- `string.concat(...) returns (string memory)`:
  `Concatenates variable number of string arguments to one string array<string-concat>`{.interpreted-tex
  role="ref"}
...
:   compute `(x * y) % k` where the multiplication is performed with
    arbitrary precision and does not wrap around at `2**256`. Asser
...
:   compute the SHA-256 hash of the inpu

`ripemd160(bytes memory) returns (bytes20)`

:   compute RIPEMD-160 hash of the inpu
...
fixed for [transaction]{#transaction}\_ signatures (see
...
or use them to identify items. OpenZeppelin has an [ECDSA helper
library](https://docs.openzeppelin.com/contracts/4.x/api/utils#ECDSA)
...
:   send given amount of Wei to `address`{.interpreted-text role="ref"},
    returns `false` on failure, forwards 2300 gas stipend, no
    adjustable
...
For more information, see the section on `address`{.interpreted-tex
role="ref"}.
...
`extcodesize` opcode when performing external calls. This ensures tha
the contract that is about to be called either actually exists (i
contains code) or an exception is raised.

The low-level calls which operate on addresses rather than contrac
...
:   Destroy the current contract, sending its funds to the given
    `address`{.interpreted-text role="ref"} and end execution. Note tha
...
:   Memory byte array that contains the runtime bytecode of the
    ...
    bytecode. Also note that libraries modify their runtime bytecode a
