# Solidity IR-based Codegen Changes {#ir-breaking-changes}
...
option `{"viaIR": true}` in standard-json and we encourage everyone to
try it out!

For several reasons, there are tiny semantic differences between the old
and the IR-based code generator, mostly in areas where we would no
expect people to rely on this behavior anyway. This section highlights
...
- The order of state variable initialization has changed in case of
  ...
  - Evaluate base constructor arguments from most derived to most base
    contract.
  - Initialize all state variables in the whole inheritance hierarchy
  ...
  This causes differences in contracts where the initial value of a
  state variable relies on the result of the constructor in another
  contract:
...
- When storage structs are deleted, every storage slot that contains a
  member of the struct is set to zero entirely. Formerly, padding space
  was left untouched. Consequently, if the padding space within a struc
  is used to store data (e.g. in the context of a contract upgrade), you
  have to be aware that `delete` will now also clear the added member
  (while it wouldn\'t have been cleared in the past).
  ...
  contract C {
      ...
      function f() public {
          // ...
          delete s;
          // s occupies only first 16 bytes of the 32 bytes slo
          // delete will write zero to the full slo
      }
  }
  ```

  We have the same behavior for implicit delete, for example when array
  of structs is shortened.
...
- Function modifiers are implemented in a slightly different way
  ...
  has an effect if the placeholder `_;` is evaluated multiple times in a
  modifier. In the old code generator, each function parameter and
  return variable has a fixed slot on the stack. If the function is run
  multiple times because `_;` is used multiple times or used in a loop,
  then a change to the function parameter\'s or return variable\'s value
  is visible in the next execution of the function. The new code
  generator implements modifiers using actual functions and passes
  function parameters on. This means that multiple evaluations of a
  function\'s body will get the same values for the parameters, and the
  effect on return variables is that they are reset to their defaul
  (zero) value for each execution.
  ...
  - Old code generator: `1` as the return variable is initialized to `0`
    only once before the first `_;` evaluation and then overwritten by
    the `return 1;`. It is not initialized again for the second `_;`
...
- For the old code generator, the evaluation order of expressions is
  ...
  order (left to right), but do not guarantee it. This can lead to
  ...
  - New code generator: `4` (`2 + 2`) but the return value is no
    guaranteed
...
- The new code generator imposes a hard limit of `type(uint64).max`
  (`0xffffffffffffffff`) for the free memory pointer. Allocations tha
...
### Internal function pointers {#internal-function-pointers-in-ir}
...
The compiler is free to omit internal functions that are never
explicitly referenced by name. As a consequence, assigning to a function
type variable in inline assembly does not guarantee that the assigned
value will be included in the internal dispatch. The function must also
...
not perform a cleanup after the bit-not assignment (`a = ~a`). This
