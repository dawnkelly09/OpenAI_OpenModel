...
The following table describes the cleaning rules applied to differen
types, where `higher bits` refers to the remaining bits in case the type
has less than 256 bits.
  ...
  signed integers higher bits set currently silently
                  to the sign bit signextends to a valid
  ...
  integers        zeroed          to a valid value, i.e.
                                  all higher bits are se
                                  to zero; may throw an
                                  exception in the future
  ---------------------------------------------------------

Note that valid and invalid values are dependent on their type size.
Consider `uint8`, the unsigned 8-bit type, which has the following valid
values:
...
The compiler will `signextend` the sign bit, which is 1 for negative and
0 for positive values, overwriting the higher bits:
