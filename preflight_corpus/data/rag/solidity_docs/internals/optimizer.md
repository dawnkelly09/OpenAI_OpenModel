::: index
optimizer, optimiser, common subexpression elimination, constan
propagation
:::

# The Optimizer {#optimizer}

The Solidity compiler involves optimizations at three different levels
(in order of execution):

- Optimizations during code generation based on a direct analysis of
  Solidity code.
...
The codegen-based optimizer affects the initial low-level code produced
from the Solidity input. In the legacy pipeline, the bytecode is
generated immediately and most of the optimizations of this kind are
implicit and not configurable, the only exception being an optimization
which changes the order of literals in binary operations. The IR-based
pipeline takes a different approach and produces Yul IR closely matching
the structure of the Solidity code, with nearly all optimizations
deferred to the Yul optimizer module. In that case codegen-level
optimization is done only in very limited cases which are difficult to
handle in Yul IR, but are straightforward with the high-level
information from analysis phase at hand. An example of such an
optimization is the bypass of checked arithmetic when incrementing the
counter in certain idiomatic `for` loops.
...
Solidity source. Similarly, one can use
...
Some optimizer steps, such as, for example, the [peephole
optimizer](https://en.wikipedia.org/wiki/Peephole_optimization) and the
`unchecked loop increment optimizer <unchecked-loop-optimizer>`{.interpreted-tex
role="ref"} are always enabled by default and can only be turned off via
the `Standard JSON <compiler-api>`{.interpreted-text role="ref"}.
::::
...
An empty optimizer sequence, i.e `:`, is accepted even withou
`--optimize` in order to fully disable the user-supplied portion of the
Yul `optimizer sequence <selecting-optimizations>`{.interpreted-tex
role="ref"}, as by default, even when the optimizer is not turned on,
the `unused pruner <unused-pruner>`{.interpreted-text role="ref"} step
will be run.
::::
...
output of the flags `--asm` and `--asm --optimize`). However, when i
comes to the Yul/intermediate-representation, there can be significan
...
a list of already known expressions. If no such matches are found, i
...
is, however, an exception to this erasing, when we can easily see tha
...
After this process, we know which expressions have to be on the stack a
...
be computed from inputs), we have to erase all knowledge about the inpu
...
Since Solidity version 0.8.2, there is another optimizer step tha
...
Inlining like this is attempted, whenever a heuristics suggests tha
...
The Yul-based optimizer consists of several stages and components tha
...
and their sequence below.
  ...
  -------------- ------------------------------------------------------------------
  `f`            `block-flattener`{.interpreted-text role="ref"}
  `l`            `circular-references-pruner`{.interpreted-text role="ref"}
  `c`            `common-subexpression-eliminator`{.interpreted-text role="ref"}
  `C`            `conditional-simplifier`{.interpreted-text role="ref"}
  `U`            `conditional-unsimplifier`{.interpreted-text role="ref"}
  `n`            `control-flow-simplifier`{.interpreted-text role="ref"}
  `D`            `dead-code-eliminator`{.interpreted-text role="ref"}
  `E`            `equal-store-eliminator`{.interpreted-text role="ref"}
  `v`            `equivalent-function-combiner`{.interpreted-text role="ref"}
  `e`            `expression-inliner`{.interpreted-text role="ref"}
  `j`            `expression-joiner`{.interpreted-text role="ref"}
  `s`            `expression-simplifier`{.interpreted-text role="ref"}
  `x`            `expression-splitter`{.interpreted-text role="ref"}
  `I`            `for-loop-condition-into-body`{.interpreted-text role="ref"}
  `O`            `for-loop-condition-out-of-body`{.interpreted-text role="ref"}
  `o`            `for-loop-init-rewriter`{.interpreted-text role="ref"}
  `i`            `full-inliner`{.interpreted-text role="ref"}
  `g`            `function-grouper`{.interpreted-text role="ref"}
  `h`            `function-hoister`{.interpreted-text role="ref"}
  `F`            `function-specializer`{.interpreted-text role="ref"}
  `T`            `literal-rematerialiser`{.interpreted-text role="ref"}
  `L`            `load-resolver`{.interpreted-text role="ref"}
  `M`            `loop-invariant-code-motion`{.interpreted-text role="ref"}
  `m`            `rematerialiser`{.interpreted-text role="ref"}
  `V`            `ssa-reverser`{.interpreted-text role="ref"}
  `a`            `ssa-transform`{.interpreted-text role="ref"}
  `t`            `structural-simplifier`{.interpreted-text role="ref"}
  `r`            `unused-assign-eliminator`{.interpreted-text role="ref"}
  `p`            `unused-function-parameter-pruner`{.interpreted-text role="ref"}
  `S`            `unused-store-eliminator`{.interpreted-text role="ref"}
  `u`            `unused-pruner`{.interpreted-text role="ref"}
  `d`            `var-decl-initializer`{.interpreted-text role="ref"}
...
others that were already applied, so repeating steps is often
beneficial.
...
An important thing to note, is that there are some hardcoded steps tha
...
addition, the delimiter may be placed at the beginning of the
user-supplied sequence, which will result in the optimization sequence
being empty, whereas conversely, if placed at the end of the sequence,
will be treated as an empty cleanup sequence.
...
as it is performed after the disambiguation stage. The reason is tha
...
FunctionHoister. Its effect is that all topmost elements that are no
function definitions are moved into a single block which is the firs
...
LoopInvariantCodeMotion, since invariants in the loop-invarian
...
value once they are defined. The advantage of this property is tha
...
The final program should be in an *expression-split form*, where (with
the exception of loop conditions) function calls cannot appear nested
inside expressions and all function call arguments have to be variables.
...
expressions or re-organize the \"expression tree\". The drawback is tha
such code is much harder to read for humans.

#### SSATransform {#ssa-transform}

This stage tries to replace repeated assignments to existing variables
...
(variables that are declared with value and never re-assigned are no
...
assigned to and at the end of the `for` loop init block if it is
assigned inside the `for` loop body or post block. If a variable\'s
value is cleared according to the rule above and the variable is
declared outside the block, a new SSA variable will be created at the
location where control flow joins, this includes the beginning of loop
post/body block and the location right after `if`/`switch`/`for`/block
statement.

After this stage, the UnusedAssignEliminator is recommended to remove
the unnecessary intermediate assignments.

This stage provides best results if the ExpressionSplitter and the
CommonSubexpressionEliminator are run right before it, because then i
does not generate excessive amounts of variables. On the other hand, the
CommonSubexpressionEliminator could be more efficient if run after the
SSA transform.

#### UnusedAssignEliminator {#unused-assign-eliminator}
...
The UnusedAssignEliminator removes all the three assignments to `a`,
because the value of `a` is not used and thus turn this snippet into
strict SSA form:
...
unused or not are connected to joining control flow.
...
At points where control flow splits, a copy of the mapping is handed
...
as state after the loop. Since `f` just has a range of three differen
...
values of variables and the call-constant state of the environment. Mos
...
The DataflowAnalyzer is not an optimizer step itself but is used as a
...
that is currently assigned to each other variable. Upon each assignmen
...
This step uses the DataflowAnalyzer and replaces subexpressions tha
...
them will always be unused. The UnusedPruner or the
...
The ExpressionSimplifier will be able to perform better replacements if
the CommonSubexpressionEliminator was run right before it.

#### ExpressionSimplifier {#expression-simplifier}

The ExpressionSimplifier uses the DataflowAnalyzer and makes use of a
...
even if their current value might not be, the ExpressionSimplifier is
...
#### CircularReferencesPruner {#circular-references-pruner}
...
The ConditionalSimplifier inserts assignments to condition variables if
...
- after `if` statement with terminating control-flow, inser
  `<condition> := 0`

Future features:

- allow replacements by `1`
- take termination of user-defined functions into accoun
...
- replace `switch` with only default case with `pop(expression)` and
  body
...
`leave`, `return`, `invalid`, `break`, `continue`, `selfdestruct`,
`revert` or by a call to a user-defined function that recurses
infinitely.

Function definitions are retained as they might be called by earlier
code and thus are considered reachable.

Because variables declared in a `for` loop\'s init block have their
scope extended to the loop body, we require ForLoopInitRewriter to run
before this step.
...
CommonSubexpressionEliminator, because SSA will make sure that the
variables will not change and the CommonSubexpressionEliminator reuses
...
It also removes declarations of variables that are never referenced. If
a declaration assigns a value that is not movable, the expression is
...
- replace `for` loop with false condition by its initialization par
...
ExpressionSplitter and SSATransform should be run upfront to obtain
better results.
...
Other optimization steps will be able to make more simplifications to
the function. The optimization step is mainly useful for functions tha
...
#### UnusedStoreEliminator {#unused-store-eliminator}

Optimizer component that removes redundant `sstore` and memory store
statements. In case of an `sstore`, if all outgoing code paths rever
(due to an explicit `revert()`, `invalid()`, or infinite recursion) or
lead to another `sstore` for which the optimizer can tell that it will
overwrite the first store, the statement will be removed. However, if
there is a read operation between the initial `sstore` and the revert,
or the overwriting `sstore`, the statement will not be removed. Such
read operations include: external calls, user-defined functions with any
storage access, and `sload` of a slot that cannot be proven to differ
from the slot written by the initial `sstore`.
...
will be transformed into the code below after the UnusedStoreEliminator
step is run
...
Example: The function to be inlined has the form of
...
The FullInliner replaces certain calls of certain functions by the
function\'s body. This is not very helpful in most cases, because i
just increases the code size but does not have a benefit. Furthermore,
code is usually very expensive and we would often rather have shorter
code than more efficient code. In some cases, though, inlining a
...
FunctionHoister and ExpressionSplitter are recommended as prerequisites
since they make the step more efficient, but are not required for
correctness. In particular, function calls with other function calls as
arguments are not inlined, but running ExpressionSplitter beforehand
ensures that there are no such calls in the input.
...
This is the opposite operation of the ExpressionSplitter. It turns a
...
The snippet `let x := add(0, 2) let y := mul(x, mload(2))` is no
...
ignored. Because of that, the snippe
...
This is a tiny step that helps in reversing the effects of the
SSATransform if it is combined with the CommonSubexpressionEliminator
and the UnusedPruner.

The SSA form we generate is detrimental to code generation because i
produces many local variables. It would be better to just reuse existing
variables with assignments instead of fresh variable declarations.
...
`a` was referenced. The SSATransform changes statements of this form by
just swapping out the declaration and the assignment. The above snippe
is turned into
...
by `a` (until `a` is re-assigned). The UnusedPruner will then eliminate
the variable `a_1` altogether and thus fully reverse the SSATransform.
...
expression did not change between the point of assignment and the poin
of use. The main benefit of this stage is that it can save stack slots
if it leads to a variable being eliminated completely (see below), bu
it can also save a `DUP` opcode on the EVM if the expression is very
cheap.

The Rematerialiser uses the DataflowAnalyzer to track the current values
...
Currently, the codegen-based optimizer module provides two
optimizations.

The first one, available in the legacy code generator, moves literals to
the right side of commutative binary operators, which helps exploi
their associativity.

The other one, available in the IR-based code generator, enables the use
of unchecked arithmetic when generating code for incrementing the
counter variable of certain idiomatic `for` loops. This avoids wasting
gas by identifying some conditions that guarantee that the counter
variable cannot overflow. This eliminates the need to use a verbose
unchecked arithmetic block inside the loop body to increment the counter
variable.

### Unchecked Loop Increment {#unchecked-loop-optimizer}

Introduced in Solidity `0.8.22`, the overflow check optimization step is
concerned with identifying the conditions under which the `for` loop
counter can be safely incremented without overflow checks.

This optimization is **only** applied to `for` loops of the general
form:
...
The condition and the fact that the counter variable is only ever
incremented guarantee that it never overflows. The precise requirements
for the loop to be eligible for the optimization are as follows:

- The loop condition is a comparison of the form `i < Y`, for a local
  counter variable `i` (called the \"loop counter\" hereon) and an
  expression `Y`.
- The built-in operator `<` is necessarily used in the loop condition
  and is the only operator that triggers the optimization. `<=` and the
  like are intentionally excluded. Additionally, user-defined operators
  are **not** eligible.
- The loop expression is a prefix or postfix increment of the counter
  variable, i.e, `i++` or `++i`.
- The loop counter is a local variable of a built-in integer type.
- The loop counter is **not** modified by the loop body or by the
  expression used as the loop condition.
- The comparison is performed on the same type as the loop counter,
  meaning that the type of the right-hand-side expression is implicitly
  convertible to the type of the counter, such that the latter is no
  implicitly widened before the comparison.
...
In this case, the counter `i` has its type implicitly converted from
`uint8` to `uint16` before the comparison and the condition is in fac
never false, so the overflow check for the increment cannot be removed.
