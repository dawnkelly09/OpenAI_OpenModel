...
In addition to releases, we provide **nightly development builds** to
make it easy for developers to try out upcoming features and provide
early feedback. Note, however, that while the nightly builds are usually
very stable, they contain bleeding-edge code from the development branch
and are not guaranteed to be always working. Despite our best efforts,
they might contain undocumented and/or broken changes that will no
become a part of an actual release. They are not meant for production
use.
...
Internet, go to
<https://github.com/ethereum/remix-live/tree/gh-pages#readme> and follow
the instructions on that page. Remix is also a convenient option for
testing nightly builds without installing multiple Solidity versions.

Further options on this page detail installing command-line Solidity
compiler software on your computer. Choose a command-line compiler if
you are working on a larger contract or if you require more compilation
options.
...
to access the compiler described further down this page. The
`commandline-compiler`{.interpreted-text role="ref"} documentation
...
The command-line options of `solcjs` are not compatible with `solc` and
tools (such as `geth`) expecting the behavior of `solc` will not work
with `solcjs`.
::::
...
from the `ethereum` organization. Use the `stable` tag for the lates
released version, and `nightly` for potentially unstable changes in the
`develop` branch.

The Docker image runs the compiler executable so that you can pass all
...
Specific compiler versions are supported as the Docker image tag such as
`ethereum/solc:0.8.23`. We will be passing the `stable` tag here instead
of specific version tag to ensure that users get the latest version by
default and avoid the issue of an out-of-date version.
::::
...
using the compiler with tooling). When using this interface, it is no
...
have to be
`loaded by the import callback <initial-vfs-content-standard-json-with-import-callback>`{.interpreted-tex
role="ref"}).
...
Binary packages of Solidity are available a
...
as AUR packages: [solidity](https://aur.archlinux.org/packages/solidity)
and [solidity-bin](https://aur.archlinux.org/packages/solidity-bin).
...
are user-produced content and unofficial packages. Exercise caution when
using them.
::::

There is also a [snap package](https://snapcraft.io/solc), however, i
...
build-from-source version. Pre-built bottles are currently no
supported.
...
``` bash
git clone https://github.com/ethereum/homebrew-ethereum.gi
...
We maintain a repository containing static builds of past and curren
compiler versions for all supported platforms a
[solc-bin](https://github.com/ethereum/solc-bin/). This is also the
location where you can find the nightly builds.

The repository is not only a quick and easy way for end users to ge
...
- Content is served with correct `Content-Type` headers and lenient CORS
  configuration so that it can be directly loaded by tools running in
  the browser.
- Binaries do not require installation or unpacking (exception for older
  Windows builds bundled with necessary DLLs).
- We strive for a high level of backward-compatibility. Files, once
  added, are not removed or moved without providing a symlink/redirec
  at the old location. They are also never modified in place and should
  always match the original checksum. The only exception would be broken
  or unusable files with the potential to cause more harm than good if
  left as is.
- Files are served over both HTTP and HTTPS. As long as you obtain the
  file list in a secure way (via git, HTTPS, IPFS or just have it cached
  locally) and verify hashes of the binaries after downloading them, you
  do not have to use HTTPS for the binaries themselves.
...
convention changes and we do not add builds for platforms that were no
...
representing a single platform. Each one includes a `list.json` file
...
- You can find the binary in the same directory under the name
  ...
  Note that the file might be a symlink, and you will need to resolve i
  yourself if you are not using git to download it or your file system
  does not support symlinks.
- The binary is also mirrored a
  ...
  HTTP redirect.
- The file is also available on IPFS a
  [QmTLs5MuLEWXQkths41HiACoXDiH8zxyqBHGFDRSzVE5CS](https://gateway.ipfs.io/ipfs/QmTLs5MuLEWXQkths41HiACoXDiH8zxyqBHGFDRSzVE5CS).
  Please, be aware that the order of items in the `urls` array is no
  predetermined or guaranteed and users should not rely on it.
- You can verify the integrity of the binary by comparing its keccak256
  ...
  utility provided by [sha3sum](https://github.com/maandree/sha3sum) or
  [keccak256() function from
...
- Use `emscripten-wasm32/` (with a fallback to `emscripten-asmjs/`)
  ...
  but the original asm.js files remain in `bin/`. The new ones had to be
  placed in a separate directory to avoid name clashes.
- Use `emscripten-asmjs/` and `emscripten-wasm32/` instead of `bin/` and
  `wasm/` directories if you want to be sure whether you are downloading
  a wasm or an asm.js binary.
- Use `list.json` instead of `list.js` and `list.txt`. The JSON lis
  format contains all the information from the old ones and more.
- Use <https://binaries.soliditylang.org> instead of
  <https://solc-bin.ethereum.org>. To keep things simple we moved almos
  everything related to the compiler under the new `soliditylang.org`
  domain and this applies to `solc-bin` too. While the new domain is
  recommended, the old one is still fully supported and guaranteed to
...
The binaries are also available a
...
The following are dependencies for all builds of Solidity:
  ...
  (version 3.21.3+ on Windows, 3.13+
  otherwise)

  [Boost](https://www.boost.org)         C++ libraries.
  (version 1.77+ on Windows, 1.67+
  otherwise)
...
Solidity versions prior to 0.5.10 can fail to correctly link agains
...
Starting from 0.5.10 linking against Boost 1.70+ should work withou
manual intervention.
::::
...
they were written for. This does not mean that a build using a differen
version is faulty. If you pass `-DSTRICT_Z3_VERSION=OFF` option to
CMake, you can build with any version that satisfies the requiremen
...
By default the build is performed in *pedantic mode*, which enables
extra warnings and tells the compiler to treat all warnings as errors.
This forces developers to fix warnings as they arise, so they do no
...
them](https://github.com/ethereum/solidity/issues/new).
::::
...
installed](https://developer.apple.com/xcode/resources/). This contains
...
X. If you are installing Xcode for the first time, or have jus
...
Solidity:
  ...
  Tools](https://visualstudio.microsoft.com/vs/older-downloads/#visual-studio-2019-and-other-products)
...
``` bash
git clone --recursive https://github.com/ethereum/solidity.gi
cd solidity
```

If you want to help develop Solidity, you should fork Solidity and add
your personal fork as a second remote:

``` bash
git remote add personal git@github.com:[username]/solidity.gi
```
...
`-DBoost_ROOT="deps/boost" -DBoost_INCLUDE_DIR="deps/boost/include"` and
...
Solidity can optionally use SMT solvers, namely `z3`, `cvc5` and
`Eldarica`, but their presence is checked only at runtime, they are no
...
The emscripten builds require Z3 and will statically link against i
instead.
::::
...
pre-release tag equals to the SemVer pre-release and the Solidity commi
...
This behavior works well with the
