...
In Solidity, this is even more important because you can use smar
...
to take too much care, but if you manage your bank account using tha
...
also machine-readable. Note that there is a [Bug Bounty
Program](https://ethereum.org/en/bug-bounty/) that covers the code
...
Using random numbers in smart contracts is quite tricky if you do no
want block builders to be able to cheat.

### Reentrancy

Any interaction from a contract (A) with another contract (B) and any
transfer of Ether hands over control to that contract (B). This makes i
...
The problem is not too serious here because of the limited gas as par
...
following contract will allow an attacker to refund multiple times as i
...
parameters before modifying the contract\'s state (Checks); only then i
makes any changes to the state (Effects); it may make calls to functions
in other contracts *after* all planned state changes have been written
to storage (Interactions). This is a common foolproof way to preven
*reentrancy attacks*, where an externally called malicious contract can
double-spend an allowance, double-withdraw a balance, among other
...
the complete contract to be stalled at a certain point. This may no
...
part of on-chain operations and stall those. Please be explicit abou
such cases in the documentation of your contracts.

### Sending and Receiving Ether

- Neither contracts nor \"externally-owned accounts\" are currently able
  to prevent someone from sending them Ether. Contracts can react on and
  reject a regular transfer, but there are ways to move Ether withou
  creating a message call. One way is to simply \"mine to\" the contrac
  address and the second way is using `selfdestruct(x)`.
- If a contract receives Ether (without a function being called), either
  the `receive Ether <receive-ether-function>`{.interpreted-tex
  role="ref"} or the `fallback <fallback-function>`{.interpreted-tex
  ...
  contract can only rely on the \"gas stipend\" it is passed (2300 gas)
  being available to it at that time. This stipend is not enough to
  modify storage (do not take this for granted though, the stipend migh
  change with future hard forks). To be sure that your contract can
  receive Ether in that way, check the gas requirements of the receive
  and fallback functions (for example in the \"details\" section in
  Remix).
- There is a way to forward more gas to the receiving contract using
  ...
  up the ability for the recipient to perform more expensive actions
  (and it returns a failure code instead of automatically propagating
  the error). This might include calling back into the sending contrac
  or other state changes you might not have thought of. So it allows for
...
- If you want to send Ether using `address.transfer`, there are certain
  ...
  1.  If the recipient is a contract, it causes its receive or fallback
  ...
  2.  Sending Ether can fail due to the call depth going above 1024.
      ...
      force the transfer to fail; take this possibility into account or
      use `send` and make sure to always check its return value. Better
      yet, write your contract using a pattern where the recipient can
      withdraw Ether instead.
  3.  Sending Ether can also fail because the execution of the recipien
      contract requires more than the allotted amount of gas (explicitly
      by using `require <assert-and-require>`{.interpreted-tex
      role="ref"}, `assert <assert-and-require>`{.interpreted-tex
      role="ref"}, `revert <assert-and-require>`{.interpreted-tex
      role="ref"} or because the operation is too expensive) - it \"runs
      out of gas\" (OOG). If you use `transfer` or `send` with a return
      value check, this might provide a means for the recipient to block
      progress in the sending contract. Again, the best practice here is
      to use a `"withdraw"
      pattern instead of a "send" pattern <withdrawal_pattern>`{.interpreted-tex
      role="ref"}.

### Call Stack Depth

External function calls can fail at any time because they exceed the
...
the expression stack are unrelated, even though both have a size limi
of 1024 stack slots.
...
measures in place, it is best to build your contract system such tha
...
Never use `tx.origin` for authorization. Let\'s say you have a walle
contract like this:
...
If your wallet had checked `msg.sender` for authorization, it would ge
the address of the attack wallet, instead of the owner\'s address. Bu
by checking `tx.origin`, it gets the original address that kicked off
the transaction, which is still the owner\'s address. The attack walle
instantly drains all your funds.

### Two\'s Complement / Underflows / Overflows {#underflow-overflow}

As in many programming languages, Solidity\'s integer types are no
actually integers. They resemble integers when the values are small, bu
...
The Solidity type `mapping` (see `mapping-types`{.interpreted-tex
role="ref"}) is a storage-only key-value data structure that does no
...
dynamic storage array, deleting or popping the array will have no effec
...
`readMap(4, 128)` returns 256. If we call `eraseMaps`, the length of the
...
Updating the code of your contract may
`invalidate the values of variables of internal function
types<function-type-value-stability-across-contract-updates>`{.interpreted-tex
role="ref"}. Consider such values ephemeral and avoid storing them in
state variables. If you do, you must ensure that they never persis
across code updates and are never used by other contracts having access
to the same storage space as a result of a delegatecall or accoun
abstraction.
...
- Types that do not occupy the full 32 bytes might contain \"dirty
  ...
  `msg.data` - it poses a malleability risk: You can craft transactions
  that call a function `f(uint8 x)` with a raw byte argument of
  `0xff000001` and with `0x00000001`. Both are fed to the contract and
  both will look like the number `1` as far as `x` is concerned, bu
  `msg.data` will be different, so if you use `keccak256(msg.data)` for
...
Keep your contracts small and easily understandable. Single ou
unrelated functionality in other contracts or into libraries. General
recommendations about the source code quality of course apply: Limit the
amount of local variables, the length of functions and so on. Documen
your functions so that others can see what your intention was and
whether it is different than what the code does.

### Use the Checks-Effects-Interactions Pattern

Most functions will first perform some checks and they should be done
first (who called the function, are the arguments in range, did they
send enough Ether, does the person have tokens, etc.).
...
While making your system fully decentralized will remove any
...
features, hands over control to a fixed and trusted third party or jus
converts the contract into a simple \"give me back my Ether\" contract.
