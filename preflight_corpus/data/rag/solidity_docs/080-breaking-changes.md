...
- Arithmetic operations revert on underflow and overflow. You can use
  `unchecked { ... }` to use the previous wrapping behavior.
...
- ABI coder v2 is activated by default.
  ...
  The pragma `pragma experimental ABIEncoderV2;` is still valid, but i
  is deprecated and has no effect. If you want to be explicit, please
  ...
  more expensive and it can also make contract calls revert that did no
  revert with ABI coder v1 when they contain data that does not conform
...
- Failing assertions and other internal checks like division by zero or
  arithmetic overflow do not use the invalid opcode but instead the
  revert opcode. More specifically, they will use error data equal to a
  function call to `Panic(uint256)` with an error code specific to the
  circumstances.

  This will save gas on errors while it still allows static analysis
  tools to distinguish these situations from a revert on invalid input,
  like a failing `require`.
...
- If constants are used in array length expressions, previous versions
  ...
  expressions, their values will be properly rounded in the same way as
  when they are used in run-time expressions.

- The type `byte` has been removed. It was an alias of `bytes1`.

## New Restrictions

This section lists changes that might cause existing contracts to no
compile anymore.

- There are new restrictions related to explicit conversions of
  literals. The previous behavior in the following cases was likely
  ambiguous:

  1.  Explicit conversions from negative literals and literals larger
      than `type(uint160).max` to `address` are disallowed.
  2.  Explicit conversions between literals and an integer type `T` are
      only allowed if the literal lies between `type(T).min` and
  ...
  4.  Explicit conversions between literals and `address` type (e.g.
      ...
      `address payable`. One can get a payable address type by using an
...
- There are new restrictions on explicit type conversions. The
  ...
  sign (signed integer to unsigned integer). In order to do the
  conversion, one has to go through an intermediate type. In the
  previous example, this would be `uint16(uint8(int8))` or
  `uint16(int16(int8))`. Note that the two ways to convert will produce
  different results e.g., for `-1`. The following are some examples of
  conversions that are disallowed by this rule.
  ...
  - `payable(uint160)`, `payable(bytes20)` and
    ...
    `payable(address(bytes20))` and `payable(address(integer-literal))`
    respectively. Note that `payable(0)` is valid and is an exception to
    the rule.
  ...
  - `Contract(uint)`: converting both type-category and width. Replace
    this by `Contract(address(uint160(uint)))`.

  These conversions were disallowed to avoid ambiguity. For example, in
  the expression `uint16 x = uint16(int8(-1))`, the value of `x` would
...
- The global functions `log0`, `log1`, `log2`, `log3` and `log4` have
  ...
  These are low-level functions that were largely unused. Their behavior
...
- Remove support for the `\b`, `\f`, and `\v` escape sequences in code.
  They can still be inserted via hexadecimal escapes, e.g. `\x08`,
...
- The global variables `tx.origin` and `msg.sender` have the type
  ...
  not these addresses are payable or not, so it now requires an explici
...
- Explicit conversion into `address` type always returns a non-payable
  ...
  - `address(u)` where `u` is a variable of type `uint160`. One can
    convert `u` into the type `address payable` by using two explici
    conversions, i.e., `payable(address(u))`.
  - `address(b)` where `b` is a variable of type `bytes20`. One can
    convert `b` into the type `address payable` by using two explici
  ...
  - `address(c)` where `c` is a contract. Previously, the return type of
    ...
    (either by having a receive function or a payable fallback
    function). The conversion `payable(c)` has the type
    `address payable` and is only allowed when the contract `c` can
    receive Ether. In general, one can always convert `c` into the type
    `address payable` by using the following explicit conversion:
    `payable(address(c))`. Note that `address(this)` falls under the
    same category as `address(c)` and the same rules apply for it.
...
- Negate unsigned integers by subtracting them from the maximum value of
  the type and adding 1 (e.g. `type(uint256).max - x + 1`, while
  ensuring that `x` is not zero)
