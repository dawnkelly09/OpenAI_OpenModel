...
The solution proposed in this EIP is to charge gas only for memory that is actually written to a
...
then, at the beginning of the opcode, gas for growing memory is only charged for `input_start + input_size`, but no
...
VM implementers should take care not to grow the memory until the end of the call and after a check that sufficien
