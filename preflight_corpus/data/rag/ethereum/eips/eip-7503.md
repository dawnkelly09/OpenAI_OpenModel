...
The EIP proposes to add a minting functionality to Ethereum, so that people can re-min
...
in case $r$ is a random value. So the funds sent to a random Ethereum address $r$ is unspendable, bu
...
Using the help of Zero-Knowledge proofs, we can hide the value of $s$! We just need to prove tha
