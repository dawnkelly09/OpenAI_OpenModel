...
home for all standards related to Ethereum. The community was small and mos
...
becoming more visible. There are changes we could make to the process that migh
...
certain ERC "categories" (a distinction that doesn't even technically exis
...
ERC repository will be affiliated with all remaining areas such as smar
...
Let's consider the impact of specification ambiguity, the impacts are differen
...
protocol EIP adjusting the gas cost for transaction data needs to be rolled ou
at a specific time uniformly across the network. Whereas a new RPC to suppor
...
fork in the third step, a consideration that simply does not apply to a smar
...
sometimes both. There is no EIP that would be shipped in mainnet that would no
...
group of wallets to collaborate on a protocol outside AllWalletDevs. Smar
...
There has been no measurable impact on the divergence of the community. Mos
...
editors, bringing the pain level down to a more acceptable level, this does no
...
as a rejection of those alternatives. To quote a famous internet meme "Why No
Both?"
...
Is an animal weakened when cells split and specialize into different tasks? I
...
This proposal only addresses the EIP and ERC proposal process and is no
