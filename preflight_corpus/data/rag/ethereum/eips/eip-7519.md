...
chains, however there is a lack of parallel primitives within the EVM to suppor
any model other than optimistic concurrency control (OCC). By adding concurren
increment and decrement operations more advanced parallel environments can be
introduced in Layer 2 networks.

This also provides the opportunity to serve the principal use case of incremen
...
post-conditions, and re-evaluating the transactions if those invariants did no
...
balances and counters while allowing for functional read support ensuring tha
...
### Opcode Instead of System Contrac

One alternative, particularly viable for Layer 2 chains, would be to implemen
...
The use of revert to handle over/underflow represents a new halt condition tha
