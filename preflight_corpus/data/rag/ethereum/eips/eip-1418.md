---
eip: 1418
title: Blockchain Storage Rent Paymen
...
* **`SENDRENT(address, amount)`** -- G_BASE -- Convert value to rent and send to accoun
  1. `σ[account]_rent` += amoun
  2. `σ[msg.sender]_balance` -= amoun
...
PAYRENT(account)
    ...
    if σ[account]_rent < 0
    		σ[account]_value += σ[account]_ren
    		σ[account]_rent = 0
    end
