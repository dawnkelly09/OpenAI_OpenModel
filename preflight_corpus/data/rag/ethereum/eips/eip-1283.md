...
* If *current value* does not equal *new value*
  ...
  * If *original value* does not equal *current value* (this storage
    ...
    * If *original value* is not 0
      * If *current value* is 0 (also means that *new value* is no
        0), remove 15000 gas from refund counter. We can prove tha
        refund counter will never go below 0.
      * If *new value* is 0 (also means that *current value* is no
...
The new gas cost scheme for `SSTORE` is divided into three differen
types:

* **No-op**: the virtual machine does not need to do anything. This is
  the case if *current value* equals *new value*.
* **Fresh**: this storage slot has not been changed, or has been rese
  to its original value. This is the case if *current value* does no
...
0), then removes this refund from the refund counter. If we didn'
issue the refund but it applies now (*new value* is 0), then adds this
refund to the refund counter. It is not possible where a refund is no
...
* We don't suffer from the optimization limitation of
  ...
  possible to know a storage's original value and current value, bu
  it is not possible to iterate over all storage changes withou
...
* It covers all usages for a transient storage. Clients that are easy
  ...
  refactoring on this. Nonetheless, no extra memory or processing cos
...
* From A to A. The previous gas cost is `200 * (N-1)`. The curren
  gas cost is `200 + 200 * (N-1)`. It satisfy *Case I*.
* From A to B. The previous gas cost is `200 * (N-1)`. The curren
...
* From Y to Z. The previous gas cost is `5000 + 200 * (N-2)`. The
  current absolute gas cost is `200 - 15000 + 5000 + 200 * (N-2)`. I
  satisfy *Case II*.
