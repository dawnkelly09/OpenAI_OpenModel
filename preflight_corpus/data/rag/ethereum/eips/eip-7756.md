...
description: Updates EIP-3155 tracing to add EOF suppor
...
EIP-3155 defined a tracing standard for Legacy EVM operations. However, the EVM Objec
...
- Clients may OPTIONALLY output Number where a Hex-Number is expected and vice-versa. Clients tha
...
- The `pc` value is zero indexed from the beginning of the contract when the contract is not in an
  ...
  magic byte. The first line of execution will not be at `"pc":0` but at the first byte of the firs
...
- `depth` starts at 1.
- `error` SHOULD be omitted if a prior CALL series or CREATE series operation within the curren
...
- `returnData` SHOULD NOT be present if a CALL series operation has not completed within the curren
frame.
...
- The `storage` member SHOULD only include items read or written via `SSTORE` or `SLOAD`, and no
...
`t8n` and to allow for more efficient RPC tracing calls, but that seemed sufficiently different tha
