...
* Sub-call, SLOAD, sub-call again, revert the inner sub-call, SLOAD the same storage slo
* SSTORE the same storage slot {1, 2, 3} times, using all combinations of zero/nonzero for original value and the value being se
* SSTORE then SLOAD the same storage slo
...
2. Applications relying on contract calls that consume close to the full gas limi
