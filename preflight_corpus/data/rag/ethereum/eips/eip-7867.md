...
* MUST reject (with error code `UNSUPPORTED_LEVEL`) batches containing at leas
  one critical call if the batch requests an atomicity level the wallet canno
...
>   `wallet_getCallsStatus` MUST return an object with a `receipts` field tha
...
Unlike a plain EOA, a shielded mempool can provide additional guarantees abou
...
included. Apps should encode deadlines and timeout behaviors in the smar
