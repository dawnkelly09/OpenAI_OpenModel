...
This defines a method of hard forking while maintaining the exac
...
against some particular contracts, then the scheme defined here migh
...
In Ethereum, a contract has a deployment method, either by a contrac
...
We let a family of contracts to always have the same `version`. Tha
...
This section defines how other EIPs might use this accoun
...
This introduces account versioning via a new RLP item in accoun
state. The design above gets account versioning by making the contrac
...
* **EIP-1891**: Instead of writing version field into account RLP
  ...
  same thing as this EIP and potentially reduces code complexity, bu
  the drawback is that every code execution will require an additional
  trie traversal, which impacts performance.

## Backwards Compatibility

Account versioning is fully backwards compatible, and it does no
...
The source of this specification can be found a
