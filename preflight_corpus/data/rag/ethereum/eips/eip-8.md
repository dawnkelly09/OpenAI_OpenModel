...
packets. When sending the hello packet, the version element should be set to the highes
...
* If such a packet is received by a node with lower version, it will blindly assume tha
...
The relaxation of discovery packet decoding rules largely codifies current practice. Mos
existing implementations do not care about the number of list elements (an exception being
go-ethereum) and do not reject nodes with mismatching version. This behaviour is no
...
if decrypt(packet) {
    ...
    // process as new forma
}
```

The plain text size prefix is perhaps the most controversial aspect of this document. I
...
recommendation to randomise the lengths is followed, pure pattern-based packe
...
* For an attacker who can afford to correlate many criteria, the size prefix would ease
  ...
  be expected to read or participate in RLPx Discovery traffic, which would be sufficien
