...
potential chain or loop of delegations, clients must retrieve only the firs
...
execution that needs extensive testing, and may be used for purposes no
...
the runtime costs of around `150`. So nearly `500` additional gas would be spen
...
directly in the transaction or to specify a pointer to the code. The simples
...
relatively a small subset. The EIP does not require adding any opcodes, tha
...
Allowing `tx.origin` to set code and execute its own delegated code enables wha
is called self-sponsoring. It allows users to take advantage of EIP-7702 withou
relying on any third party infrastructure.

However, that means the EIP breaks the invariant that `msg.sender == tx.origin`
only happens in the topmost execution frame of a transaction. This will affec
...
2. Protecting against atomic sandwich attacks like flash loans, which rely on
   the ability to modify state before and after the execution of the targe
...
This distribution of occurrences—many (1), some (2), and no (3)—is exactly wha
the authors of this EIP expect because:

* Determining if `msg.sender` is an EOA without `tx.origin` is difficult, if no
impossible.
* The only execution context which is safe from atomic sandwich attacks is the
topmost context, and `tx.origin == msg.sender` is the only way to detect tha
context.
...
The `PER_AUTH_BASE_COST` is the cost to process the authorization tuple and se
...
unavoidable. Even if a user has a zeroed out delegation indicator, mos
...
possible to have an external storage contract that the smart contract walle
...
This EIP does not provide developers the opportunity to run initcode and se
...
smart contract wallet developers must verify the initial calldata to the accoun
...
[ERC-7201](./eip-7201.md) a contract may root its storage layout at a slo
dependent on a unique identifier. To simplify this, smart contract languages may
provide a way of re-rooting the entire storage layout of existing contrac
source code.
...
authorization (i.e., increasing the account's nonce) or by sweeping the relevan
