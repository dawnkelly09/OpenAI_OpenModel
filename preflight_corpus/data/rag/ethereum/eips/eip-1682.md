...
The `rentblock` field tracks the block number in which the `rent balance` was las
...
If all these preconditions are met, `RESTORETO` transfers the storage of the accoun
...
accounting, leaving it unable to pay back users accurately if the threshold balance isn'
reached.
...
The proposed changes tries to fit within the existing state transition model. Note tha
there is no mechanism for deactivating accounts the moment they can't pay rent. Users mus
