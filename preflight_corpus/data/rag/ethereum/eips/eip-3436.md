...
The current specification of Clique allows for multiple competing blocks from producers but does no
...
`header_number` is the number of the header, and `validator_count` is the count of the curren
...
```
(header_number - validator_index) % validator_coun
```
...
For the second scenario with the same validator set and in-order chain with validator 7 having jus
...
scenario the odd and even chains would produce a block for 7 and 8 respectively, and chain hal
would result with no validators that have not chosen a fork. Only a manual rollback would fix this.

One consideration when formulating the rules is that the block choice should be chosen so that i
...
Malicious and motivated attackers who are participating in the network can force the chain to hal
with well crafted block production. With a fully deterministic choice rule the opportunity to hal
...
makes such replacements more likely. However the impact of such an attack seems low to trivial a
this time.
