...
* the call does not count against the block's gas limi
...
### Beacon block root instead of state roo
...
ring buffer and verifies the timestamp matches the one originally used to write the root data when being read. Given the fixed size of storage slots (only 32 bytes), the requiremen
