...
class AccountDataPhase:
    """
    The account next conversion step continues migrating the accoun
    code (if any) and basic data. After processing, the account mus
...
def get_conversion_account(state: State) -> CurrentConvertingAccount:
    # When starting the conversion, initialize with the first accoun
    # in the MPT.
    ...
    return state._conversion_curr_accoun
...
def state_convert(state: State, stride: int):
    ...
    while n < stride and not state._conversion_finished:
        ...
        # Account code and basic data.
            ...
            # get_account will pull from MPT and then we write to the VKT. If the accoun
            ...
            # Thus, this operation is correct under both scenarios. That is, it won'
            # write stale data.
            account = get_account(state, curr_account.address)
...
### Not counting [EIP-161](./eip-161.md) accounts for `CONVERSION_STRIDE` limi
