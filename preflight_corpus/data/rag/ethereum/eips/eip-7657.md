...
class SyncCommitteeSlashingEvidence(Container):
    ...
    actual_finalized_block_root: Roo
...
class SyncCommitteeSlashing(Container):
    ...
    recent_finalized_block_root: Roo
    recent_finalized_slot: Slo
...
def is_valid_sync_committee_slashing_evidence(evidence: SyncCommitteeSlashingEvidence,
    ...
    if evidence.actual_finalized_block_root == Root():
        ...
        finalized_slot = evidence.finalized_header.slo
        if recent_finalized_slot < finalized_slot:
            return False
        distance = recent_finalized_slot - finalized_slo
        if distance == 0:
...
def process_sync_committee_slashing(state: BeaconState, sync_committee_slashing: SyncCommitteeSlashing) -> None:
    ...
    assert state.slot >= evidence_1.signature_slot > evidence_1.attested_header.slot >= evidence_1.finalized_header.slo
    assert state.slot >= evidence_2.signature_slot > evidence_2.attested_header.slot >= evidence_2.finalized_header.slo
    assert evidence_1.attested_header.slot >= evidence_2.attested_header.slo
    ...
    # It is not allowed to sign conflicting `attested_header` for a given slo
    ...
    recent_finalized_slot = sync_committee_slashing.recent_finalized_slo
    recent_finalized_block_root = sync_committee_slashing.recent_finalized_block_roo
    if (
    ...
    if recent_finalized_block_root == Root():
        ...
        # `actual_finalized_branch` may be rooted in the provided `finalized_header` with highest slo
        rooted_in_evidence_1 = (
            evidence_1.finalized_header.slot >= evidence_2.finalized_header.slo
            and recent_finalized_slot == evidence_1.finalized_header.slo
            and recent_finalized_block_root == evidence_1.actual_finalized_block_root and linear_1
        )
        rooted_in_evidence_2 = (
            evidence_2.finalized_header.slot >= evidence_1.finalized_header.slo
            and recent_finalized_slot == evidence_2.finalized_header.slo
