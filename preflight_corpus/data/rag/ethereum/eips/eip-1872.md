...
`Roadmap` network upgrades that include deliberate protocol improvements. Nex
are `Priority` network updates, where there are technical reasons tha
...
Scheduling details such as whether this choice is made prior to or after testne
...
upgrades or for security risks that have a existing mitigation in place tha
...
promptly after a solution to the systemic risk is agreed upon between the clien
implementors.
...
To continue this pattern network upgrade activation block numbers for mainne
...
Block numbers for Roadmap and Priority network upgrades should be chosen so tha
...
frame within which to perform internal upgrades free from external changes. I
