...
1. Creation of a non-contract account by sending non-zero ETH to an address with no associated accoun
2. Creation of a non-contract account by the block with `coinbase` pointing to an address with no associated accoun
3. Creation of a non-contract account by `SELFDESTRUCT` with the argument being an address with no associated accoun
