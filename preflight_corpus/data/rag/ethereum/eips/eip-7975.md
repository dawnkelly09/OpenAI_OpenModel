...
If the `lastBlockIncomplete` flag is set to true (`1`), the last receipt list does no
...
For a `Receipts` message, each block receipts list is validated by checking the full lis
