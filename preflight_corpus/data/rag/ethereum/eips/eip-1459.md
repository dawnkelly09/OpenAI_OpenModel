...
lists requires a software update. The current lists are small, giving the clien
...
secp256k1 private key. The corresponding public key must be known to the clien
...
protocol. Entries of the merkle tree are contained in DNS TXT records. The roo
...
- `enrtree://<key>@<fqdn>` is a leaf pointing to a different list located a
...
3. Resolve the TXT record of the hash subdomain, e.g.
   "CFZUWDU7JNQR4VTCZVOJZ5ROV4.mynodes.org" and verify whether the conten
   matches the hash.
...
resolved to avoid going into an infinite loop. It's in the client's bes
...
CloudFlare DNS, dnsimple, Amazon Route 53 using their respective clien
libraries.

### Why is this a merkle tree?

Being a merkle tree, any node list can be authenticated by a single signature on
the root. Hash subdomains protect the integrity of the list. At wors
intermediate resolvers can block access to the list or disallow updates to it,
but cannot corrupt its content. The sequence number prevents replacing the roo
with an older version.

Synchronizing updates on the client side can be done incrementally, which
matters for large lists. Individual entries of the tree are small enough to fi
...
enable client implementations to sync these trees independently. A clien
