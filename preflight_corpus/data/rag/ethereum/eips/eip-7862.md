...
class Header:
    ...
    prev_randao: Bytes32
    nonce: Bytes8
    base_fee_per_gas: Uin
    withdrawals_root: Roo
    blob_gas_used: U64
    excess_blob_gas: U64
    parent_beacon_block_root: Roo
```
...
def validate_header(chain: BlockChain, header: Header) -> None:
    ...
    # Validate delayed state root matches the last computed state roo
...
def state_transition(chain: BlockChain, block: Block) -> None:
    ...
    # This will be included in the next block as its state_roo
...
1. Clients MUST validate the block's structure including the delayed state roo
...
def apply_fork_transition(chain: BlockChain) -> None:
    ...
    """
    # At fork activation, initialize last_computed_state_roo
...
2. **Faster slot times**: Reduced slot duration while maintaining the same gas limi
