...
address size from 20 to 32 bytes"). One issue ground the work to a halt: EVM opcodes that accep
...
When future uses of address space extension are specified it is expected that the exceptional hal
...
operation specified in [EIP-2929](./eip-2929.md). This means if the account is no
...
Also, for the `EXTCALL`, `EXTDELEGATECALL`, and `EXTSTATICCALL` operations a new step is added jus
...
Preparing for Address Space Expansion is explicitly done inside the scope of EOF with the inten
