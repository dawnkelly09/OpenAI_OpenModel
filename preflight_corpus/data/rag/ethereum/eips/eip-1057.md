...
hash32_t keccak_f800_progpow(uint32_t* state)
{
    ...
    // Squeeze phase for fixed 8 words of outpu
    hash32_t ret;
    for (int i=0; i<8; i++)
        ret.uint32s[i] = st[i];
...
* When mining this final value is compared against a `hash32_t` targe
...
)
{
    ...
    */
    ...
    // Reduce mix data to a per-lane 32-bit diges
    ...
    // Reduce all lanes to a single 256-bit diges
    for (int i = 0; i < 8; i++)
