...
class World(ABC):
	def validate_block(self, block: Block) -> None:
		...
		# check if the gas limit is at least the minimum gas limi
		assert block.gas_limit >= 5000

		# check if the base fee is correc
		if INITIAL_FORK_BLOCK_NUMBER == block.number:
			expected_base_fee_per_gas = INITIAL_BASE_FEE
		elif parent_gas_used == parent_gas_target:
			expected_base_fee_per_gas = parent_base_fee_per_gas
		elif parent_gas_used > parent_gas_target:
			gas_used_delta = parent_gas_used - parent_gas_targe
		...
		for unnormalized_transaction in transactions:
			...
			signer.balance -= transaction.amoun
			...
			# priority fee is capped because the base fee is filled firs
