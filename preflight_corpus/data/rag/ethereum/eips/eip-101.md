...
self.storage[-1] += 1
# Make the sub-call and discard outpu
...
1.  Run the code for a maximum of 50000 gas, stopping if they see an operation or call that threatens to go over this limi
...
1.  In the outer scope just before calling, call the ether contract to create a cheque for the desired amoun
