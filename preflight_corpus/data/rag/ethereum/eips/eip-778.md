...
The key/value pairs must be sorted by key and must be unique, i.e. any key may be presen
...
All keys except `id` are optional, including IP addresses and ports. A record withou
...
the same port number in both `tcp`, `tcp6` or `udp`, `udp6` should be avoided but doesn'
render the record invalid.

### RLP Encoding

The canonical encoding of a node record is an RLP list of `[signature, seq, k, v, ...]`.
The maximum encoded size of a node record is 300 bytes. Implementations should rejec
...
- Adding identity schemes: these need implementation consensus because the network won'
