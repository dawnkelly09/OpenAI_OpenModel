...
* `NON_REVERT_MIN_DEPOSIT`: amount in wei configurable by clien
...
* When applying `vote` transactions to `CASPER_ADDR` to vm state:
  ...
  * Do not count gas of `vote` toward the normal `gas_used`. For all `vote` transaction receipts, cumulative gas used is equal to last non-`vote` transaction receip
...
The source code for `CASPER_CODE` is located a
