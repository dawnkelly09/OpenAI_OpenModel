 ...
 * If a signer wishes to enact a change to the list of authorized signers, it will:
   * Set the **miner** to the signer it wishes to vote abou
   * Set the **nonce** to `0` or `0xff...f` to vote in favor of adding or kicking ou
...
// Define the various voting scenarios to tes
tests := []struct {
...
}{
  {
    ...
    // Authorizations are counted once per signer per targe
    signers: []string{"A", "B"},
    blocks:  []block{
      {signer: "A", voted: "C", auth: true},
      {signer: "B"},
    ...
    // Deauthorizations are counted once per signer per targe
    signers: []string{"A", "B"},
    blocks:  []block{
      {signer: "A", voted: "B", auth: false},
      {signer: "B"},
