...
When a packet of this type is received, the node should reply with an ENRResponse packe
...
should be handled as for all other existing packets i.e. no reply should be sent if i
...
The recipient of the packet should verify that the node record is signed by node who sen
ENRResponse.
