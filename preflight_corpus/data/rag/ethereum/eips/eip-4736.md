...
5. A single change operation for a single validator, with all required fields in the file with no other content presen
...
* User A: Controls the CL keys and the EL key used for the deposi
* User B: Controls the CL keys, but does not control the EL key for the deposi
