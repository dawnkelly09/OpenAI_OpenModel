...
`ECADD` (at address `0x06`), `ECMUL` (at address `0x07`), and pairing check (a
address `0x08`) precompiled contracts was shifted to [Cloudflare's bn256
library](https://github.com/cloudflare/bn256). Based on the [initial PR tha
...
EIP-196, while the gas costs for the pairing check are updates to the cos
listed in EIP-197. Updated gas costs have been adjusted to the less performan
