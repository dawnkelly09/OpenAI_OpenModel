...
This specification makes the following assumptions about the documen
lifecycle.
...
be included. Custom fields **should** be prefixed with `x-` to preven
...
is not integral in nature for package installation, but may be importan
...
The `contractTypes` field hosts the [Contrac
...
-   Packages **should not** include abstract contracts in the contrac
...
as the [Contract Types](#contract-type) and other deploymen
...
To reference the address of a [Contrac
...
-   This value **may not** reference the same contract instance tha
...
-   The `<contract-instance>` value **must** be a valid [Contrac
...
The `linkDependencies` defines the [Link Values](#link-value) tha
...
The `description` field provides additional detail that may be relevan
...
The `contractName` field defines the [Contrac
Name](#contract-name) for this [Contrac
Type](#contract-type).
...
The `deploymentBytecode` field defines the bytecode for this [Contrac
Type](#contract-type).
...
portion of [Bytecode](#bytecode) for this [Contrac
Type](#contract-type).
...
### The *Contract Instance* Objec

A **Contract Instance Object** represents a single deployed [Contrac
Instance](#contract-instance) and is defined to have the following
key/value pairs.

#### Contract Type: `contractType`

The `contractType` field defines the [Contrac
Type](#contract-type) for this [Contrac
...
To reference a contract type from this Package, use the forma
`<contract-alias>`.

-   The `<contract-alias>` value **must** be a valid [Contrac
    Alias](#contract-alias).

-   The value **must** be present in the keys of the `contractTypes`
    section of this Package.

To reference a contract type from a dependency, use the forma
`<package-name>:<contract-alias>`.

-   The `<package-name>` value **must** be present in the keys of the
    `buildDependencies` of this Package.

-   The `<contract-alias>` value **must** be be a valid [Contrac
    Alias](#contract-alias).
...
[Contract Type](#contract-type) for this [Contrac
...
### The *Compiler Information* Objec
...
#### Contract Alias
This is a name used to reference a specific [Contrac
...
The `<contract-name>` portion **must** be the same as the [Contrac
...
#### Contract Instance
A contract instance a specific deployed version of a [Contrac
Type](#contract-type).
...
A name which refers to a specific [Contrac
...
In cases where there is a single deployed instance of a given [Contrac
Type](#contract-type), package managers **should** use the
[Contract Alias](#contract-alias) for that contract type for this
name.

In cases where there are multiple deployed instances of a given contrac
...
#### Contract Name
The name found in the source code that defines a specific [Contrac
...
a malicious package that disguises itself as a trusted package with similar but differen
