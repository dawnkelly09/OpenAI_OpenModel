...
* Reify tokens on-chain via smart contrac
...
* Contained: users do not need to understand the proof mechanism used by the particular token implementation contrac
...
Moves token(s) from a message to a contrac
...
* A byte array can be decoded as a struct and checked for errors within the implementation contrac
