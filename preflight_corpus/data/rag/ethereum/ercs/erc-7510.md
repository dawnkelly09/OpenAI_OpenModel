...
```solidity
/// @notice The struct used to reference a token in an NFT contrac
struct Token {
    address collection;
    uint256 id;
}
...
interface IERC7510 {
    ...
    /// @param otherToken Another token to check as a parent or no
    ...
    /// @param parentTokens The parent tokens to se
