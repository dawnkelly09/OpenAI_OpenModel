...
struct ResolvedCrossChainOrder {
	...
	///      a floor on filler receipts. Setting the `recipient` of an `Output` to address(0) indicates that the filler is no
...
/// @notice Tokens that must be received for a valid order fulfillmen
struct Output {
	/// @dev The address of the ERC20 token on the destination chain
	/// @dev address(0) used as a sentinel for the native token
	bytes32 token;
	/// @dev The amount of the token to be sen
	uint256 amount;
	/// @dev The address to receive the output tokens
	bytes32 recipient;
	/// @dev The destination chain for this outpu
	uint256 chainId;
}
...
interface IOriginSettler {
	...
	/// @dev This method must emit the Open even
...
2. It must be filled on the correct destination contrac
