...
User Interfaces which are generated programmatically from the ABI of the token contrac
...
A contract MAY register either its address or the address of another contrac
...
*NOTE*: A regular address MAY register a different address&mdash;the address of a contract&mdash;implementing
the interface on its behalf.
A contract MUST register either its address or the address of another contrac
but said address MUST implement the interface on its behalf.
...
such that other EIPs and smart contract infrastructures can benefit from i
...
The difference for new contracts implementing [ERC-20] is tha
