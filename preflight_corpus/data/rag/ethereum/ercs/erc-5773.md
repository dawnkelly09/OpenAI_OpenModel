...
interface IERC5773 /* is ERC165 */ {
    ...
    /**
     * @notice Used to notify listeners that an asset object at `assetId` is added to token's pending asse
     *  array.
     * @param tokenIds An array of IDs of the tokens that received a new pending asse
     ...
     * @param tokenId ID of the token for which to accept the pending asse
     * @param index Index of the asset in the pending array to accep
     ...
     * @dev The priority array is a non-sequential list of `uint16`s, where the lowest value is considered highes
     *  priority.
     ...
     * @dev Asset priorities are a non-sequential array of uint16 values with an array size equal to active asse
     *  priorites.
     ...
     * @param operator Address of the account that we are checking whether it has the operator role or no
