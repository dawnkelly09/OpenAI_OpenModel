...
* If the UserOperation is valid, the client MUST return the calculated `userOpHash` for i
* in case of failure, MUST return an `error` result object, with `code` and `message`.
  ...
  * **code: -32501** - transaction rejected by `paymaster` contract's `validatePaymasterUserOp` function
    * The `message` field SHOULD be set to the revert message from the `paymaster` contrac
  ...
  * **code: -32503** - UserOperation out of time-range:
    ...
    * The `data` field SHOULD contain a `paymaster` address if this error was triggered by the `paymaster` contrac
...
users are not required to provide the same `eip7702Auth` tuple for any consequen
`UserOperation`.
...
* **success** boolean - whether this execution completed without a rever
