...
interface IEIP6366Core {
   ...
   * The function SHOULD revert if the message caller’s account permission does not have the subse
   ...
   * `approve()` method SHOULD `revert` if granting `_permission` permission is no
   ...
   * @param _permission     Checking permission se
...
interface IEIP6366Error {
  ...
  /**
   * Conflict between permission se
   */
