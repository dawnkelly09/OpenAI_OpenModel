...
* The `ownerOf` function MAY be implemented as `pure` or `view`, and MUST return the address of the original owner of
  the NFT.
* The `recipientOf` function MAY be implemented as `pure` or `view`, and MUST return the address of the account tha
  received the role.
...
functionality was introduced to allow third-parties to interact with ERC-7432 without requiring NFT ownership. Complian
