...
/// @notice   Move amount from payer's token balance to deposite balance to ensure payment is sufficien
function deposit(uint256 amount) external;

/**
 *  @notice   Give remaining deposite balance back to "to" account, act as "refund" function
 *  @dev      In prepayment module, withdraw is executed from issuer accoun
 *            In lock-release module, withdraw is executed from user accoun
...
sign(keccak256(abi_encode(
    "\x19Ethereum Signed Message:\n32",
        keccak256(abi_encode(
            ...
            payer_address,
            token_issuer,
            token_consumption,        //calculated by user clien
            epoch
        ))
    ))
,private_key)
...
serving_loop:
    ...
    for {
         ...
         * signed_consumption is claimable amoun
...
- Token is more standardized and has better UI suppor
