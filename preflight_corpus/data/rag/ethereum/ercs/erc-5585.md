...
interface IERC5585 {
    ...
    /// @notice Get all available rights of this NFT projec
    ...
    /// @dev The zero address indicates there is no user. It will throw exception when the rights are not defined by this NFT projec
    /// @param tokenId The NFT which is authorized
    /// @param user The user to whom the NFT is authorized
    ...
    /// @notice resetAllowed flag can be updated by contract owner to control whether the authorization can be revoked or no
    ...
    /// @return true or false whether the NFT is available for authorization or no
