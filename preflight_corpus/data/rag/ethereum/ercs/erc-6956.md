...
description: Asset-bound NFTs anchor a token 1-1 to an asset and operations are authorized through oracle-attestation of control over the asse
...
- **ANCHOR-TECHNOLOGY** MUST ensure tha
  - the ANCHOR is inseparable from the ASSET (physically or otherwise)
  - an ORACLE can establish PROOF-OF-CONTROL over the ASSET beyond reasonable doub
  - For physical ASSETS, additional [Security considerations for Physical Assets](#security-considerations-for-physical-assets) MUST be taken into accoun
 ...
 * @notice Asset-bound Non-Fungible Tokens anchor a token 1:1 to a (physical or digital) asset and token transfers are authorized through attestation of control over the asse
...
interface IERC6956 {
    ...
    enum Authorization {
        ...
        ISSUER,             // = (1<<ISSUER), // The issuer of the tokens, i.e. this smart contrac
        ...
        ALL                 // = (1<<OWNER) | (1<<ISSUER) | (1<<ASSET) // Owner + Issuer + Asse
    }
...
- MUST implement `decodeAttestationIfValid(attestation, data)` to validate and decode ATTESTATIONS as specified in the [ORACLE-Section](#oracle)
  ...
  - MUST throw when
    ...
    - ATTESTATION has expired or is not valid ye
...
interface IERC6956AttestationLimited is IERC6956 {
  ...
  /// @dev MUST return the global attestation limit per defaul
  ///      and override the global attestation limit in case an anchor-based limit is se
  ...
  /// @dev Is computed by comparing the attestationsUsedByAnchor(anchor) and the current attestation limi
...
interface IERC6956ValidAnchors is IERC6956 {
    ...
    /**
     * @notice Indicates whether an anchor is valid in the present contrac
     * @dev Typically implemented via MerkleTrees, where proof is used to verify anchor is part of the MerkleTree
     *      MUST return false when no ValidAnchorsUpdate-event has been emitted ye
...
- required ... we don't see a way how to implement the use-case without i
...
- A Minimal Typescript sample to generate an ATTESTATION using ethers library is available [here](../assets/eip-6956/minimalAttestationSample.ts)
