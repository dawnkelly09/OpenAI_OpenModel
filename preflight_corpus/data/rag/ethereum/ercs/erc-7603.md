...
interface IERC7603 /* is ERC165 */ {
    ...
    /**
     * @notice Used to notify listeners that an asset object at `assetId` is added to token's asse
     *  array.
     * @param tokenId An ID of the token that received a new asse
     ...
     * @dev Asset priorities are a non-sequential array of uint16 values with an array size equal to asse
     *  priorites.
