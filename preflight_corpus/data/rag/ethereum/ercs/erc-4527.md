...
; the path of the associated key, even if not present in the `components` elemen
; of this structure.
    crypto-keypath = {
        components: [path-component], ; If empty, source-fingerprint MUST be presen
...
eth-sign-request = (
    ...
    derivation-path: #5.304(crypto-keypath), ;the key path for signing this reques
    ?request-id: uuid, ; the uuid for this signing reques
