...
interface IUniversalIdentity {
    ...
    struct HardwareIdentity {
        bytes32 publicKey;            // Hardware-bound public key uniquely tied to this robo
        string manufacturer;           // Identifier for the robot's manufacturer
        string operator;               // Identifier for the robot's operator
        string model;                  // Model identifier of the robo
        string serialNumber;           // Unique serial number for the robo
...
contract UniversalIdentity is IUniversalIdentity, OwnableUpgradeable {
    ...
    /// @notice Subscribe and register to a specific UniversalCharter contract using its stored rule se
    /// @param charter The address of the UniversalCharter contrac
    ...
    function subscribeAndRegisterToCharter(address charter, uint256 version) external {
        ...
        // Register as a robot in the charter using the fetched rule se
        UniversalCharter(charter).registerUser(IUniversalCharter.UserType.Robot, ruleSet);

        emit SubscribedToCharter(charter);
    }

    /// @notice Leave the system for a specific UniversalCharter contrac
    ...
    function leaveCharter(address charter) external {
        ...
        // Call the leaveSystem function of the UniversalCharter contrac
    ...
    /// @notice Checks if the robot has agreed to follow a specific rule and if it is complian
    /// @param rule The rule to check.
    /// @return bool Returns true if the robot has agreed to the rule and is complian
...
contract UniversalCharter is IUniversalCharter, OwnableUpgradeable {
    ...
    /// @notice Variable to track the current version of the rule se
    uint256 private currentVersion;

    /// @notice Variable to store the address of the SystemConfig contrac
    ...
    /// @notice Registers a user (either human or robot) by agreeing to a rule se
    /// @param userType The type of user: Human or Robo
    /// @param ruleSet The array of individual rules the user agrees to follow
    function registerUser(UserType userType, bytes[] memory ruleSet) external override whenNotPaused {
        ...
        // For robots, ensure compliance with each rule via the UniversalIdentity contrac
        if (userType == UserType.Robot) {
            require(_checkRobotCompliance(msg.sender, version), "Robot not compliant with rule set");
        }

        // Register the user with the versioned rule se
    ...
    function leaveSystem() external override whenNotPaused {
        ...
        // For robots, verify compliance with all rules in the rule se
    ...
    /// @notice Checks if a user complies with their registered rule se
    /// @param user The address of the user (human or robot)
    /// @param ruleSet The array of individual rules to verify
    /// @return bool Returns true if the user complies with the given rule se
    function checkCompliance(address user, bytes[] memory ruleSet) external view override returns (bool) {
        ...
        // For robots, check compliance with each rule in the UniversalIdentity contrac
    ...
    /// @param robotAddress The address of the robo
    /// @param version The version of the rule set to verify compliance with
    /// @return bool Returns true if the robot is compliant with all the rules in the rule se
    ...
    function updateRuleSet(bytes[] memory newRuleSet) external whenNotPaused onlyOwner {
        ...
        // Increment the version and store the new rule se
        currentVersion += 1;
