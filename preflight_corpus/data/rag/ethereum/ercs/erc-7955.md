...
### Use of CREATE2 Factory Contrac
...
# Copy the init code to memory offset 0. Note that if the call to the contrac
# incorrectly encoded, this will revert with "out of gas", as it will attemp
...
0x000e: CREATE2         # Stack: [address; 0; 32]               | Do `create2(v, code, salt)`, which leaves the address
                                                                # of the contract on the stack, or 0 if the contrac
                                                                # creation reverted
...
0x0014: DUP1            # Stack: [0; r.len; 0; 0; 32]           | Push 0 on to the stack by duplicating; note tha
                                                                # `PUSH0` is intentionally do not used for increased
                                                                # compatibility with chains that do not implement tha
                                                                # specific opcode
