...
- **MUST** be able to force transfers from an Agent walle
...
wallet address is in the `Identity Registry` of the token, and if the `Identity`contract linked to his walle
...
interface ICompliance {
    ...
    // functions
    // initialization of the compliance contrac
    function bindToken(address _token) external;
    function unbindToken(address _token) external;

    // check the parameters of the compliance contrac
