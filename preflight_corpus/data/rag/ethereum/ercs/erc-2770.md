---
eip: 2770
title: Meta-Transactions Forwarder Contrac
...
```
`from` - an externally-owned account making the reques
`to` - a destination address, normally a smart-contrac
...
```
`forwardRequest` - an instance of the `ForwardRequest` struc
...
`suffixData` - RLP-encoding of the remainder of the request struc
...
Note that `gas` parameter behaves according to EVM rules, specifically EIP-150. The forwarder validates internally tha
