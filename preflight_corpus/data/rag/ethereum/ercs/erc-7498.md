...
- onchain: token mints with context of items spen
- offchain: the ability to associate with ecommerce orders (through `redemptionHash`)
- trait redemptions: improving the burn-to-redeem experience with [ERC-7496](./eip-7496.md) Dynamic Traits.
...
Including trait redemptions is optional, but if the token would like to enable trait redemptions the token MUST include [ERC-7496](./eip-7496.md) Dynamic Traits.
...
[ERC-7496](./eip-7496.md) Dynamic Traits is used for trait redemptions to support onchain enforcement of trait values for secondary market orders.
...
If trait redemptions are desired, tokens implementing this EIP must properly implement [ERC-7496](./eip-7496.md) Dynamic Traits.
