...
The standard interface implemented by tokens built on Ethereum is called ERC-20, and Contracts includes a widely used implementation of it: the aptly named [`ERC20`](api:token/ERC20.html) contract. This contract, like the standard itself, is quite simple and bare-bones. In fact, if you try to deploy an instance of `ERC20` as-is it will be quite literally useless…​ it will have no supply! What use is a token with no supply?
...
Starting with Contracts v2, this pattern is not only discouraged, but disallowed. The variables `totalSupply` and `balances` are now private implementation details of `ERC20`, and you can’t directly write to them. Instead, there is an internal [`_mint`](api:token/ERC20.html#ERC20-_mint-address-uint256-) function that will do exactly this:
...
The internal [`_mint`](api:token/ERC20.html#ERC20-_mint-address-uint256-) function is the key building block that allows us to write ERC-20 extensions that implement a supply mechanism.
...
So far our supply mechanism was triggered manually, but `ERC20` also allows us to extend the core functionality of the token through the [`_update`](api:token/ERC20.html#ERC20-_update-address-address-uint256-) function.
...
``` highlight
link:api:example$ERC20WithAutoMinerReward.sol[role=include]
```
